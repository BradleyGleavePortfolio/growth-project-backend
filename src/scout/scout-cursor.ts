...
function value(v: unknown): v is string {
  return typeof v === 'string' && v.length > 0 && Array.from(v).length <= 256 &&
    !v.includes('\u0000') && !Array.from(v).some((c) => {
...
export function decodeScoutCursor(
  cursor: string | undefined, coach: string, intent: string, family: string,
): Boundary {
  ...
  try {
    ...
    if (!value(v.c) || !value(v.i) || !value(v.s) || v.c !== coach ||
        v.i !== intent || v.f !== family) throw new Error();
    const canonical = v2
...
export function scoutCursorOrder(after: Boundary): Prisma.ScoutReconstructionLedgerOrderByWithRelationInput |
  Prisma.ScoutReconstructionLedgerOrderByWithRelationInput[] {
  return after?.p === undefined ? { source_id: 'asc' } :

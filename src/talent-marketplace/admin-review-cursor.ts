...
export const ADMIN_REVIEW_MIN_LIMIT = 1;
...
// Clamp a client-supplied limit into [ADMIN_REVIEW_MIN_LIMIT,
// ADMIN_REVIEW_MAX_LIMIT], defaulting when omitted or NaN. The hard cap stops a
// caller from pulling an unbounded page; the NaN guard (P3-5) stops a coerced
// non-numeric query param from poisoning `take`.
export function clampReviewLimit(limit: number | undefined): number {
  if (limit === undefined || Number.isNaN(limit)) return ADMIN_REVIEW_DEFAULT_LIMIT;
  return Math.max(ADMIN_REVIEW_MIN_LIMIT, Math.min(ADMIN_REVIEW_MAX_LIMIT, Math.trunc(limit)));
}

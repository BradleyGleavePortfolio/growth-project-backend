...
function setup() {
  const row = {
    id: '99457965-6db2-4dcb-bde3-05c843b681fa', chosen_platform: 'truecoach',
    superseded_at: null, challenge: {
      code: '012345', used_at: null, failed_attempts: 0,
      expires_at: new Date(Date.now() + 120000),
    },
  };
  const prisma = withSetupTransaction({ extensionPairCode: {
  ...
  return { row, prisma, auth, svc: new ExtensionPairService(asPrismaDouble(prisma), asAuthDouble(auth)) };
}
...
describe('C1-S1 nonce recovery', () => {
  it('replays the exact ID/code/expiry without mutation or minting', async () => {
    ...
    const expected = {
      import_intent_id: row.id, pairing_code: row.challenge.code,
  ...
  it('conflicting platform returns 409 before mutation', async () => {
    ...
    await expect(svc.init('coach-1', 'other', nonce))
      .rejects.toMatchObject({ status: 409, response: { code: 'setup_nonce_conflict' } });
    expect(prisma.importIntent.updateMany).not.toHaveBeenCalled();
  });

  it.each(['expired', 'used', 'locked', 'superseded', 'retired'])(
    'unavailable challenge (%s) returns 410 without replacing it', async (reason) => {
      ...
      await expect(svc.init('coach-1', 'truecoach', nonce))
        .rejects.toMatchObject({ status: 410, response: { code: 'setup_challenge_unavailable' } });
      expect(prisma.importIntent.create).not.toHaveBeenCalled();
      expect(prisma.importIntent.updateMany).not.toHaveBeenCalled();
    },
  );
  ...
  it.each([['id'], ['coach_id', 'setup_nonce'], undefined])(
    'does not retry a non-code or unidentified unique constraint %s', async (target) => {
      const { svc, prisma } = setup();

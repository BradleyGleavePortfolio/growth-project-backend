...
const row = () => ({
  ...
  failed_attempts: 0,
  created_at: new Date(),
  import_intent_id: intent as string | null,
});
function setup() {
  const stored = row();
  const prisma = withSetupTransaction({
    ...
    extensionPairCode: {
      create: jest
        .fn()
        .mockImplementation(async ({ data }: { data: Partial<ReturnType<typeof row>> }) => {
          Object.assign(stored, data);
          return stored;
        }),
      findUnique: jest.fn().mockImplementation(async () => ({ ...stored })),
      updateMany: jest.fn().mockImplementation(async ({ where }: { where: { id?: string } }) => {
        if (!where.id || stored.used_at || stored.expires_at.getTime() <= Date.now())
          return { count: 0 };
  ...
  prisma.importIntent.findFirst.mockImplementation(async () => ({
    id: stored.import_intent_id,
    chosen_platform: stored.chosen_platform,
    paired_at: stored.used_at,
    superseded_at: null,
    challenge: stored,
  }));
  const auth = {
...
describe('C1 durable intent issuance and echo', () => {
  it('persists a fresh opaque server UUID at init, never the code or owner', async () => {
    ...
    expect(prisma.extensionPairCode.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        import_intent_id: Reflect.get(result, 'import_intent_id'),
        coach_id: 'coach-1',
      }),
    });
  ...
  it('preserves the exact init ID through status, redeem, and durable retrieval', async () => {
    ...
    expect(await svc.status('coach-1', stored.code)).toEqual({
      status: 'pending',
      import_intent_id: id,
    });
  ...
  it('expiry while auth mint is pending returns no token or paired status', async () => {
    ...
    await expect(svc.redeem('123456')).rejects.toMatchObject({
      response: { code: 'already_used' },
    });
    expect(stored.used_at).toBeNull();
    expect(await svc.status('coach-1', '123456')).toEqual({
      status: 'expired',
  ...
  it('demotion at token mint cannot mark setup paired', async () => {
    ...
    auth.mintExtensionSessionForCoach.mockRejectedValue(
      new BadRequestException({ code: 'invalid' }),
    );

...
describe('C1 durable session boundary', () => {
  ...
  it('passes the authenticated owner, never a body-supplied owner, through the controller', async () => {
    ...
    expect(
      await controller.session(authedRequest('coach-1'), { import_intent_id: intent }),
  ...
  it('storage misses return identical not-found without minting or revealing ownership', async () => {
    ...
    await expect(svc.session('coach-2', intent)).rejects.toMatchObject({
      status: 404,
      response: { message: 'Pairing session not found. Create a new pairing code.' },
    });
    expect(prisma.importIntent.findFirst).toHaveBeenCalledWith({
      where: {
        id: intent,
  ...
  it.each([
    ...
    'enforces owner/role/deletion/legacy predicates for %s %s %s %s',
    async (owner, role, deletedAt, savedId, allowed) => {
      const { svc, prisma, stored } = setup();
      prisma.importIntent.findFirst.mockImplementation(
        async ({
          where,
        }: {
          where: { coach_id: string; id: string; coach: typeof activeCoach };
        }) =>
          where.coach_id === owner &&
          where.id === savedId &&
          where.coach.role.in.includes(String(role)) &&
          where.coach.deleted_at === deletedAt
            ? stored
            : null,
      );
      if (allowed) {
  ...
  it('reads pending versus expired unused setup truthfully, never inferring pairing', async () => {
    ...
    Object.assign(stored, {
      paired_at: null,
      challenge: { expires_at: new Date(Date.now() + 120000), failed_attempts: 0 },
    });
    expect(await svc.session('coach-1', intent)).toMatchObject({
      status: 'pending',
      import_intent_id: intent,
    });
  ...
  it.each(['session', 'current'] as const)(
    'keeps %s POST, bearer-only, coach/owner guarded with the global authenticated throttle',
    (route) => {
      ...
      expect(Reflect.getMetadata('__headers__', handler)).toContainEqual({
        name: 'Cache-Control',
        value: 'no-store',
      });
      ...
      expect(() => guard.canActivate(executionContextFor({ role: 'student' }))).toThrow(
  ...
  it.each(['session', 'current'])(
    '%s is dark by default before authentication or storage work',
    (route) => {
      ...
      const req = {
        path: `/api/extension/pair/${route}`,
        originalUrl: `/api/extension/pair/${route}`,
        headers: {},
      };
  ...
  it('validates the new UUID in the real request pipe', async () => {
    ...
    for (const value of [
      ...
      await expect(
        pipe.transform({ import_intent_id: value }, { type: 'body', metatype: PairSessionDto }),
  ...
  it('rejects ownership/intent injection while preserving old init and redeem requests', async () => {
    for (const [metatype, body] of [
      [PairInitDto, { chosen_platform: 'truecoach' }],
      [PairRedeemDto, { code: '123456' }],
    ] as const) {
      expect(await pipe.transform(body, { type: 'body', metatype })).toEqual(body);
      await expect(
        pipe.transform({ ...body, import_intent_id: intent }, { type: 'body', metatype }),
      ).rejects.toMatchObject({ status: 400 });
    }
    await expect(
      pipe.transform(
        { import_intent_id: intent, coach_id: 'coach-2' },
        {
          type: 'body',
          metatype: PairSessionDto,
        },
      ),
    ).rejects.toMatchObject({ status: 400 });
  });
  ...
  it('current reads use authenticated owner, optionally recover an older nonce, and never mint', async () => {
    ...
    expect(prisma.importIntent.findFirst).toHaveBeenLastCalledWith(
  ...
  it('optional nonce is UUIDv4, not null, and current rejects authority injection', async () => {
    ...
    for (const setup_nonce of [null, '', '123456', 42]) {
      await expect(
        pipe.transform({ setup_nonce }, { type: 'body', metatype: PairCurrentDto }),
      ).rejects.toMatchObject({ status: 400 });
    }
    await expect(
      pipe.transform({ coach_id: 'other' }, { type: 'body', metatype: PairCurrentDto }),
    ).rejects.toMatchObject({ status: 400 });
    expect(
      await pipe.transform(
        { chosen_platform: 'truecoach', setup_nonce: intent },
        { type: 'body', metatype: PairInitDto },
      ),
    ).toHaveProperty('setup_nonce', intent);
  });
});

...
export class LogFoodDto {
  ...
  // Idempotency key generated by the mobile offline queue. The backend upserts
  // on this key so a flush retry after a crash does not create a duplicate entry.
  @IsOptional()
  @IsString()
  @MaxLength(128)
  client_uuid?: string;
...
export class UpdateLogEntryDto {
  ...
  // Allow the client to update the human-readable quantity + unit alongside
  // quantity_multiplier so coach views stay in sync. Both are optional—
  // omitting them preserves the existing stored value. See Fix 5.
  @IsOptional()
  @IsNumber()
  @Min(0)
  @Max(100000)

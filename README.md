...
| `ANDROID_PACKAGE_NAME` | Package name emitted in `/.well-known/assetlinks.json`. Defaults to `com.growthproject.app`; override only if a separate Play track uses a different package. | `com.growthproject.app` |
...
| `FINANCE_SERVICE_TOKEN_NEXT` | New: dual-secret rotation for the inbound federation bearer (`POST /api/admin/federation/ptm-signal`). Mirrors the `STRIPE_WEBHOOK_SECRET_NEXT` pattern. Rotation playbook: (1) generate `openssl rand -hex 32`, set this var on the fitness backend and set `FINANCE_SERVICE_TOKEN=<new>` on the finance backend, deploy both; (2) confirm `event=federation.auth.next-secret-used` in fitness logs; (3) promote — set `FINANCE_SERVICE_TOKEN=<new>` on fitness and clear `FINANCE_SERVICE_TOKEN_NEXT`, redeploy. Leave unset in normal operation. | 256-bit random hex |

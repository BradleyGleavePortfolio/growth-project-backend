...
describe('R33 STRUCTURAL half only (not the live Complete verdict) — one terminal, one basis row, same transaction, after the CAS', () => {
  it('complete → terminal from the S9 closed set + ONE ScoutRunSettledBasis row recording the settled report and the epoch digests', async () => {
    ...
    // The landed coach notice (ScoutService.notifyComplete, kind import.complete) goes to the
    // importing coach on an accepted claim; customer-facing side effects stay at zero (R38).
  ...
  it('a second claim after the terminal is a no-op ack and writes no second basis row (insert-only; one per run)', async () => {
    const intentId = await readyRun();
    expect((await complete(intentId)).failure).toBeUndefined();
    const again = await complete(intentId);
    // Landed completeServerRun contract: a duplicate settle on a terminal run is a 200 ack no-op.
...
describe('R34 — S9 codes win; a fence wins; nothing else decides the terminal', () => {
  ...
  it('a fenced run: the fence writes the terminal and NO basis; a later claim is a no-op ack and writes none', async () => {
    const intentId = await readyRun();
    ...
    // Landed completeServerRun contract: a late settle on a fenced run is a 200 ack no-op.
    expect(late.failure).toBeUndefined();
    expect(late.pushes).toBe(0);
    expect(late.queries.filter(isBasisInsert)).toHaveLength(0);
    expect(count(SETTLED)).toBe(0);
...
describe('R36 — CAS miss or insert failure: no terminal, no basis', () => {
  it('an injected ScoutRunSettledBasis insert failure rolls the terminal back: terminal_status NULL, zero basis rows, no settle event', async () => {
    ...
    // The claim committed before the settle, so a retry is a refused claim (200 ack) until the S11-B
    // re-drive; whatever the retry does, terminal and basis stay paired, never one without the other.
    const retry = await complete(intentId);
    expect(retry.failure).toBeUndefined();
    expect(count(SETTLED)).toBe(runRow(COACH, intentId).terminal_status === null ? 0 : 1);
  });

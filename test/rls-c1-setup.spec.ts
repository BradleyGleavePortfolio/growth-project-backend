...
if (parsed.protocol !== 'postgresql:' || parsed.hostname !== '127.0.0.1' ||
    parsed.port !== '55439' || parsed.pathname !== '/c1_setup_disposable' || parsed.search ||
    parsed.username !== 'user' || parsed.password) {
...
function sql(text: string): string {
  return execFileSync(psql!, ['-X', '-v', 'ON_ERROR_STOP=1', '-At', url!], {
    input: text, encoding: 'utf8', timeout: 15000, stdio: ['pipe', 'pipe', 'pipe'],
  }).trim();
}
...
const auth = { mintExtensionSessionForCoach: jest.fn(async () => ({
...
beforeAll(async () => {
  ...
  sql(migration(stage));
  expect(sql(`SELECT count(*) FROM "ExtensionPairCode" WHERE id='legacy-row' AND import_intent_id IS NULL`)).toBe('1');
...
describe('C1-S1 real PostgreSQL authority and recovery', () => {
  it('concurrent same-nonce requests and a restarted service recover one committed result', async () => {
    const key = nonce();
    const results = await Promise.all(Array.from({ length: 8 }, () => svc().init('a', 'truecoach', key)));
    expect(new Set(results.map(result => JSON.stringify(result))).size).toBe(1);
    expect(await svc().init('a', 'truecoach', key)).toEqual(results[0]);
    expect(await svc().current('a')).toMatchObject({ import_intent_id: results[0].import_intent_id, status: 'pending' });
  ...
  it('recovers a committed init after its process exits without delivering the response', async () => {
    ...
    `;
    const run = (mode: string) => execFileSync(process.execPath, [
      '-r', 'ts-node/register', '-e', child, serviceUrl.toString(), key, mode,
    ], {
      cwd: resolve(__dirname, '..'), encoding: 'utf8', timeout: 20000,
    ...
    expect(JSON.parse(run('recover'))).toEqual({
      import_intent_id: saved.id, pairing_code: saved.challenge!.code,
  ...
  it('different nonces serialize to one current setup; old replay cannot invalidate it', async () => {
    ...
    await expect(svc().init('a', 'truecoach', keys[oldIndex])).rejects.toMatchObject({ status: 410 });
    expect(await svc().current('a')).toEqual(current);
    expect((await svc().current('a', keys[oldIndex])).import_intent_id).toBe(results[oldIndex].import_intent_id);
    expect(await prisma.extensionPairCode.count({ where: { expires_at: { gt: new Date() } } })).toBe(1);
  });
  ...
  it.each(['student', 'deleted', 'missing'])('denies inactive/ineligible owner %s before writes', async (owner) => {
  ...
  it('revalidates owner on reads and at redeem commit, including demotion during mint', async () => {
    ...
    await expect(svc().redeem(setup.pairing_code)).rejects.toMatchObject({ response: { code: 'invalid' } });
    expect((await prisma.extensionPairCode.findUniqueOrThrow({ where: { code: setup.pairing_code } })).used_at).toBeNull();
    await expect(svc().session('owner', setup.import_intent_id!)).rejects.toMatchObject({ status: 404 });
  ...
  it('expires without extending TTL; exact nonce recovers ID after challenge deletion', async () => {
    ...
    expect(await svc().current('a', key)).toEqual({
      import_intent_id: setup.import_intent_id, chosen_platform: 'truecoach', status: 'expired',
    });
  });
  ...
  it('one concurrent redeem wins, atomically records pairing, and never replays tokens', async () => {
    ...
    const results = await Promise.allSettled([svc().redeem(setup.pairing_code), svc().redeem(setup.pairing_code)]);
    ...
    expect(await svc().session('a', setup.import_intent_id!)).toEqual({
      import_intent_id: setup.import_intent_id, chosen_platform: 'truecoach', status: 'paired',
    });
  });
  ...
  it('five actual code collisions roll back new intent and preserve previous setup', async () => {
    ...
    await expect(svc().init('a', 'truecoach', nonce()))
      .rejects.toMatchObject({ response: { code: 'code_mint_failed' } });
    expect(await prisma.importIntent.count()).toBe(1);
    expect((await svc().current('a')).status).toBe('pending');
  });

  it.each(['ImportIntent', 'ExtensionPairCode'])('injected %s update failure rolls back all setup writes', async (table) => {
    const original = await svc().init('a', 'truecoach', nonce());
  ...
  it('database constraints independently enforce current uniqueness and owner-consistent links', async () => {
    const setup = await svc().init('a', 'truecoach', nonce());
    await expect(prisma.importIntent.create({ data: { coach_id: 'a', chosen_platform: 'truecoach' } }))
      .rejects.toMatchObject({ code: 'P2002' });
    await expect(prisma.extensionPairCode.update({
      where: { code: setup.pairing_code }, data: { coach_id: 'b' },
    })).rejects.toMatchObject({ code: 'P2003' });
  });

  it.each(['anon', 'authenticated'])('restrictive RLS denies %s despite hostile permissive policies', async (role) => {
    await svc().init('a', 'truecoach', nonce());
    for (const table of ['ImportIntent', 'ExtensionPairCode']) {
      expect(sql(`SELECT relrowsecurity AND relforcerowsecurity FROM pg_class WHERE oid='"${table}"'::regclass`)).toBe('t');
      ...
      try {
        ...
        expect(sql(`SET ROLE ${role}; UPDATE "${table}" SET coach_id='b';`)).toBe('SET\nUPDATE 0');
        const insert = table === 'ImportIntent'
  ...
  it('legacy binary-shaped rows remain redeemable and omit ID; nonce-less init stays non-idempotent', async () => {
    await prisma.extensionPairCode.create({ data: {
      code: '000000', coach_id: 'a', chosen_platform: 'truecoach', expires_at: new Date(Date.now() + 120000),
    } });

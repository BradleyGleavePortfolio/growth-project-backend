...
describe('loadLedgerSync', () => {
  it('matches the async loader on the fixture', () => {
    const sync = loadLedgerSync(FIXTURE_PATH);
    expect(sync.version).toBe(1);
    expect(sync.entries.length).toBeGreaterThan(0);
  });

  it('throws on a missing source_path just like the async path', () => {
    const { source_path: _omit, ...withoutPath } = entry();
    const p = writeLedger({ version: 1, entries: [withoutPath] });
    expect(() => loadLedgerSync(p)).toThrow(/source_path/);
  });
});

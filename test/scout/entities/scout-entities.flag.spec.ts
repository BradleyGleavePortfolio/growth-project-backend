...
/**
 * R-DARK-1 for the IMPORTER-I entities read route.
 *
 * GET /api/scout/reconstruct/entities is a SUBPATH of /api/scout/reconstruct, so
 ...
 * middleware change and NO new flag. The route is live only when BOTH flags are
 * exactly "true"; otherwise it is a uniform 404 before any guard runs — the read
 * can never be exposed more broadly than the write it reads.
 */

const ENTITIES_PATH = '/api/scout/reconstruct/entities';
...
describe('featureFlagNotFoundMiddleware — /api/scout/reconstruct/entities (IMPORTER-I)', () => {
  ...
  it('404s when FEATURE_SCOUT_RECONSTRUCT is off even if FEATURE_SCOUT_INGEST is on', () => {
    ...
    featureFlagNotFoundMiddleware(makeReq(ENTITIES_PATH), res, next);

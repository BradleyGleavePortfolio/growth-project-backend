...
describe('ScoutReconstructService', () => {
  ...
  it.each(['P2002', 'P2034'])(
    'retries %s once on skipped and failed ledger writes',
    async (code) => {
      for (const outcome of ['skipped', 'failed']) {
        ...
        const update = jest.spyOn(prisma.scoutReconstructionLedger, 'upsert').mockRejectedValueOnce(
        ...
        expect([...prisma.ledger.values()][0]).toMatchObject({
          status: outcome,
          source_platform: outcome === 'skipped' ? 'auto:coachrx.example.com' : 'truecoach',
        });
      }
    },
  );

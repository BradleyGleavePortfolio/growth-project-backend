...
describe('T/Q0 disposable target guard', () => {
  it('requires its own confirmed target and strips only Prisma options for psql', () => {
    expect(
      g2Tq0TestTarget(`${base}?schema=public&connection_limit=2`, 'g2_tq0_disposable'),
    ).toEqual({
  ...
  it.each([
    ...
    base.replace('55439', '5432'),
    base.replace('g2_tq0_disposable', 'g2_ledger_expand_disposable'),
    base.replace('user@', 'service_role@'),
    base.replace('user@', 'user:secret@'),
    `${base}?host=remote`,
    `${base}?schema=private`,

#!/usr/bin/env bash
# S7-3' G2 R/ready proof bootstrap, derived by substitution from the accepted B bootstrap
# test/utils/g2-b-drain-bootstrap.sh (unchanged) for the R-only disposable cluster/database. Validation harness only: it never touches a hosted or
...
#   G2_R_DATABASE_URL  postgresql://r_super@127.0.0.1:<port>/g2_r_ready_disposable?schema=public&connection_limit=2
#   G2_R_CONFIRM       g2_r_ready_disposable:<port>
#   G2_R_PASSWORD      disposable local fixture password shared by r_super/postgres/service_role
#                         (never written to a file by this harness; only into process env / in-memory URLs)
#   G2_R_PSQL          absolute path of a psql binary compatible with the PG17 server
#   G2_R_OLD_ROOT      DETACHED Git checkout of the preserved O source 925780e0; create/verify it
#                         offline with test/utils/g2-pg17-old-root.sh (a `git archive` extraction is
#                         not a repository and cannot pass step 4 below)
#   G2_R_OLD_CLIENT    directory that receives the independently generated O Prisma client
# Optional:
#   G2_R_SERVER_VERSION  expected server_version_num (default 170006 = 17.6)
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OLD_HEAD=925780e0a1906593e5383c618311b6b17364b8dc
E_MIGRATION=20270118000000_scout_ledger_platform_expand
EXPECTED_VERSION="${G2_R_SERVER_VERSION:-170006}"
# Distinctive R fixture markers: pinned literals, never read from the environment (see
# test/utils/g2-r-ready-db.ts G2_R_CLUSTER_MARKER / G2_R_DATABASE_MARKER, kept identical by
# test/scout/g2-r-ready-db-guard.spec.ts). The lane's postgresql.conf must set
# cluster_name = 'r-disposable-pg17'; the disposable database carries DB_MARKER as its comment.
CLUSTER_MARKER=r-disposable-pg17
DB_MARKER=r-g2-ready-synthetic-disposable-fixture-safe-to-drop

for name in G2_R_DATABASE_URL G2_R_CONFIRM G2_R_PASSWORD G2_R_PSQL G2_R_OLD_ROOT G2_R_OLD_CLIENT; do
  [[ -n "${!name:-}" ]] || { echo "missing $name" >&2; exit 2; }
done
[[ -x "$G2_R_PSQL" ]] || { echo "psql binary not executable: $G2_R_PSQL" >&2; exit 2; }
...
[[ "$G2_R_OLD_CLIENT" == "$G2_R_OLD_ROOT"/* ]] \
  || { echo "G2_R_OLD_CLIENT must live inside G2_R_OLD_ROOT (pinned package root); got $G2_R_OLD_CLIENT" >&2; exit 2; }
...
eval "$(cd "$ROOT" && node -r ts-node/register/transpile-only -e '
  const { g2RReadyTestTarget } = require("./test/utils/g2-r-ready-db");
  const t = g2RReadyTestTarget(process.env.G2_R_DATABASE_URL, process.env.G2_R_CONFIRM);
  ...
  const { withFixturePassword } = require("./test/utils/g2-r-ready-db");
  console.log("MIGRATE_AUTH_URL=" + q(withFixturePassword(t.prismaUrl, process.env.G2_R_PASSWORD, "postgres")));
')"
export PGPASSWORD="$G2_R_PASSWORD"

psql_maint() { "$G2_R_PSQL" -X -w -qAt -v ON_ERROR_STOP=1 "$MAINT_URL" "$@"; }
psql_db() { "$G2_R_PSQL" -X -w -qAt -v ON_ERROR_STOP=1 "$PSQL_URL" "$@"; }
...
psql_maint -v pw="$G2_R_PASSWORD" <<'SQL'
DO $$ BEGIN
...
#    by the operator (DROP DATABASE g2_r_ready_disposable as the cluster superuser) first.
...
if [[ "$(psql_maint -c "SELECT count(*) FROM pg_database WHERE datname='g2_r_ready_disposable'")" == "0" ]]; then
  psql_maint -c 'CREATE DATABASE g2_r_ready_disposable OWNER postgres'
  psql_maint -v marker="$DB_MARKER" <<'SQL'
COMMENT ON DATABASE g2_r_ready_disposable IS :'marker';
SQL
fi
EXISTING_MARKER="$(psql_maint -c "SELECT COALESCE(shobj_description(oid,'pg_database'),'') FROM pg_database WHERE datname='g2_r_ready_disposable'")"
[[ "$EXISTING_MARKER" == "$DB_MARKER" ]] \
  || { echo "g2_r_ready_disposable exists without the R disposable marker comment; refusing to reuse it" >&2; exit 3; }
psql_maint -c 'ALTER DATABASE g2_r_ready_disposable OWNER TO postgres'
[[ "$(psql_db -c "SELECT count(*) FROM pg_tables WHERE schemaname='public'")" == "0" ]] \
  || { echo "g2_r_ready_disposable already has public tables; reset it explicitly before bootstrapping again" >&2; exit 3; }
...
[[ "$(git -C "$G2_R_OLD_ROOT" rev-parse HEAD)" == "$OLD_HEAD" ]] || { echo "old root is not $OLD_HEAD" >&2; exit 4; }
git -C "$G2_R_OLD_ROOT" diff --quiet HEAD -- package.json package-lock.json src prisma \
...
[[ -e "$G2_R_OLD_ROOT/node_modules" ]] || ln -s "$ROOT/node_modules" "$G2_R_OLD_ROOT/node_modules"
[[ ! -d "$G2_R_OLD_ROOT/prisma/migrations/$E_MIGRATION" ]] || { echo "old root unexpectedly contains E" >&2; exit 4; }
OLD_COUNT="$(find "$G2_R_OLD_ROOT/prisma/migrations" -mindepth 1 -maxdepth 1 -type d | wc -l)"
...
(cd "$G2_R_OLD_ROOT" && DATABASE_URL="$MIGRATE_AUTH_URL" DIRECT_URL="$MIGRATE_AUTH_URL" \
...
mkdir -p "$G2_R_OLD_CLIENT"
PINNED_CLIENT_PKG="$(cd "$ROOT" && node -p 'require("fs").realpathSync(require.resolve("@prisma/client/package.json"))')"
RESOLVED_CLIENT_PKG="$(node -p 'try { require("fs").realpathSync(require.resolve("@prisma/client/package.json", { paths: [process.argv[1]] })) } catch (e) { "UNRESOLVED" }' "$G2_R_OLD_CLIENT")"
[[ "$RESOLVED_CLIENT_PKG" == "$PINNED_CLIENT_PKG" ]] \
  || { echo "@prisma/client does not resolve from $G2_R_OLD_CLIENT to the pinned tree ($RESOLVED_CLIENT_PKG != $PINNED_CLIENT_PKG); refusing to generate" >&2; exit 6; }
...
OLD_SCHEMA_COPY="$G2_R_OLD_CLIENT/.schema-for-generate.prisma"
sed "s|provider = \"prisma-client-js\"|provider = \"prisma-client-js\"\n  output   = \"$G2_R_OLD_CLIENT\"|" \
  "$G2_R_OLD_ROOT/prisma/schema.prisma" > "$OLD_SCHEMA_COPY"
(cd "$G2_R_OLD_ROOT" && "$ROOT/node_modules/.bin/prisma" generate --schema "$OLD_SCHEMA_COPY" >/dev/null)
grep -q 'model ScoutReconstructionLedger' "$G2_R_OLD_CLIENT/schema.prisma"
O_ENGINE_SHA="$(hash_required 'generated O client engine' "$G2_R_OLD_CLIENT/$ENGINE")"
...
O_RUNTIME="$G2_R_OLD_CLIENT/runtime/library.js"
...
echo "O_CLIENT_GENERATED dir=$G2_R_OLD_CLIENT engine_sha256=$O_ENGINE_SHA runtime=$O_RUNTIME runtime_sha256=$O_RUNTIME_SHA runtime_header_bytes=$((O_RUNTIME_SIZE - PINNED_RUNTIME_SIZE)) pinned_runtime=$PINNED_RUNTIME pinned_runtime_sha256=$PINNED_RUNTIME_SHA"
! awk '/model ScoutReconstructionLedger \{/,/\}/' "$G2_R_OLD_CLIENT/schema.prisma" | grep -q source_platform \
  || { echo "generated O client unexpectedly knows source_platform" >&2; exit 6; }

# 7. Candidate (R) client in the candidate root: the R client generated once, before commit, in
#    the isolated copy of the accepted B dependency tree (R changes prisma/schema.prisma) is only
...
# The generated client must be R's (both wide keys declared), not a stale C1/B client.
for key in ScoutIngestEntity_identity_key ScoutReconstructionLedger_identity_key; do
  grep -q "map: \"$key\"" "$ROOT/node_modules/.prisma/client/schema.prisma" \
    || { echo "candidate client schema lacks $key; the R client was not generated" >&2; exit 7; }
done

if [[ "$MODE" == generate-only ]]; then echo "G2_R_GENERATE_OK"; exit 0; fi   # no connection in this mode

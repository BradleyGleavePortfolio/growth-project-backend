/**
 * G2-N/Q1 live-proof helpers layered on the N/Q1-only derivative (test/utils/g2-nq1-pg-harness.ts)
 * of the accepted R harness (test/utils/g2-r-ready-pg-harness.ts, unchanged).
 * Carries the B and R helpers unchanged (S1, C1, E, B, R are fixture history here: the T root IS
 * the accepted R head, so its `prisma migrate deploy` installs exactly that history) and adds
 * only what N/Q1 needs: the five-field ledger identity observation. N/Q1 ships no migration.
 * Imported only by the explicitly guarded test/rls-g2-nq1.spec.ts.
 */
import { PrismaClient } from '@prisma/client';
import { readFileSync } from 'fs';
import { resolve } from 'path';
import { json, quote, root, sql, target } from './g2-nq1-pg-harness';
import { G2_NQ1_RUNTIME_ROLE, withFixturePassword } from './g2-nq1-db';
...
export const narrow = () =>
  ...
  WHERE c.relname IN ('ScoutIngestEntity_coach_id_intent_id_source_id_key',
    ...
    AND c.relnamespace='public'::regnamespace`);
/** Accepted S1 and C1 migrations: both the candidate root and the T root (accepted R head) track
 *  them; the T root's `prisma migrate deploy` in the bootstrap records the whole history, so the
 *  candidate's deploy has NOTHING pending (N/Q1 ships no migration). Nothing about them is proven. */
...
/**
 * In-process runtime-role (service_role) client: the same generated N client the candidate ships,
...
export function runtimeClient(name: string) {
  const url = new URL(
    withFixturePassword(target.prismaUrl, process.env.G2_NQ1_PASSWORD, G2_NQ1_RUNTIME_ROLE),
  );

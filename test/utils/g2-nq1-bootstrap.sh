#!/usr/bin/env bash
# S7-3' G2 N/Q1 proof bootstrap, derived by substitution from the accepted R bootstrap
# test/utils/g2-r-ready-bootstrap.sh (unchanged) for the N/Q1-only disposable cluster/database.
# The "old" side is T: a detached checkout of the accepted R head 7d2895e1 (T writer, Q0 readers,
# R schema); its `prisma migrate deploy` installs the whole accepted history (S1..R) so the N/Q1
# candidate, which ships no migration, must have nothing pending. Validation harness only: it never touches a hosted or
...
#   G2_NQ1_DATABASE_URL  postgresql://nq1_super@127.0.0.1:<port>/g2_nq1_disposable?schema=public&connection_limit=2
#   G2_NQ1_CONFIRM       g2_nq1_disposable:<port>
#   G2_NQ1_PASSWORD      disposable local fixture password shared by nq1_super/postgres/service_role
#                         (never written to a file by this harness; only into process env / in-memory URLs)
#   G2_NQ1_PSQL          absolute path of a psql binary compatible with the PG17 server
#   G2_NQ1_OLD_ROOT      DETACHED Git checkout of the accepted R head 7d2895e1 (the T side); create it
#                         with `git worktree add --detach` (a `git archive` extraction is not a
#                         repository and cannot pass step 4 below)
#   G2_NQ1_OLD_CLIENT    directory that receives the independently generated T Prisma client
# Optional:
#   G2_NQ1_SERVER_VERSION  expected server_version_num (default 170006 = 17.6)
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OLD_HEAD=7d2895e1fe03ea82353e8ce0b07aacaf66af74c8
R_MIGRATION=20270120000000_scout_identity_ready
# 164 base migrations + S1 + C1 + E + B + R tracked by the T root (and by this candidate).
EXPECTED_MIGRATIONS=169
E_MIGRATION=20270118000000_scout_ledger_platform_expand
EXPECTED_VERSION="${G2_NQ1_SERVER_VERSION:-170006}"
# Distinctive R fixture markers: pinned literals, never read from the environment (see
# test/utils/g2-nq1-db.ts G2_NQ1_CLUSTER_MARKER / G2_NQ1_DATABASE_MARKER, kept identical by
# test/scout/g2-nq1-db-guard.spec.ts). The lane's postgresql.conf must set
# cluster_name = 'nq1-disposable-pg17'; the disposable database carries DB_MARKER as its comment.
CLUSTER_MARKER=nq1-disposable-pg17
DB_MARKER=nq1-g2-synthetic-disposable-fixture-safe-to-drop

for name in G2_NQ1_DATABASE_URL G2_NQ1_CONFIRM G2_NQ1_PASSWORD G2_NQ1_PSQL G2_NQ1_OLD_ROOT G2_NQ1_OLD_CLIENT; do
  [[ -n "${!name:-}" ]] || { echo "missing $name" >&2; exit 2; }
done
[[ -x "$G2_NQ1_PSQL" ]] || { echo "psql binary not executable: $G2_NQ1_PSQL" >&2; exit 2; }
...
# an output directory outside a package root made the CLI run `npm i` silently). The T client is
# generated INSIDE the T checkout, whose node_modules is the shared symlink to the pinned npm-ci tree,
...
[[ "$G2_NQ1_OLD_CLIENT" == "$G2_NQ1_OLD_ROOT"/* ]] \
  || { echo "G2_NQ1_OLD_CLIENT must live inside G2_NQ1_OLD_ROOT (pinned package root); got $G2_NQ1_OLD_CLIENT" >&2; exit 2; }
...
eval "$(cd "$ROOT" && node -r ts-node/register/transpile-only -e '
  const { g2Nq1TestTarget } = require("./test/utils/g2-nq1-db");
  const t = g2Nq1TestTarget(process.env.G2_NQ1_DATABASE_URL, process.env.G2_NQ1_CONFIRM);
  ...
  const { withFixturePassword } = require("./test/utils/g2-nq1-db");
  console.log("MIGRATE_AUTH_URL=" + q(withFixturePassword(t.prismaUrl, process.env.G2_NQ1_PASSWORD, "postgres")));
')"
export PGPASSWORD="$G2_NQ1_PASSWORD"

psql_maint() { "$G2_NQ1_PSQL" -X -w -qAt -v ON_ERROR_STOP=1 "$MAINT_URL" "$@"; }
psql_db() { "$G2_NQ1_PSQL" -X -w -qAt -v ON_ERROR_STOP=1 "$PSQL_URL" "$@"; }
...
#     not the R lane initialised for this proof; stop without creating roles or databases.
CLUSTER="$(psql_maint -c "SELECT current_setting('cluster_name')")"
[[ "$CLUSTER" == "$CLUSTER_MARKER" ]] || { echo "cluster_name '$CLUSTER' is not the R fixture marker '$CLUSTER_MARKER'; refusing to touch this server" >&2; exit 3; }
...
psql_maint -v pw="$G2_NQ1_PASSWORD" <<'SQL'
DO $$ BEGIN
...
#    by the operator (DROP DATABASE g2_nq1_disposable as the cluster superuser) first.
...
if [[ "$(psql_maint -c "SELECT count(*) FROM pg_database WHERE datname='g2_nq1_disposable'")" == "0" ]]; then
  psql_maint -c 'CREATE DATABASE g2_nq1_disposable OWNER postgres'
  psql_maint -v marker="$DB_MARKER" <<'SQL'
COMMENT ON DATABASE g2_nq1_disposable IS :'marker';
SQL
fi
EXISTING_MARKER="$(psql_maint -c "SELECT COALESCE(shobj_description(oid,'pg_database'),'') FROM pg_database WHERE datname='g2_nq1_disposable'")"
[[ "$EXISTING_MARKER" == "$DB_MARKER" ]] \
  || { echo "g2_nq1_disposable exists without the R disposable marker comment; refusing to reuse it" >&2; exit 3; }
psql_maint -c 'ALTER DATABASE g2_nq1_disposable OWNER TO postgres'
[[ "$(psql_db -c "SELECT count(*) FROM pg_tables WHERE schemaname='public'")" == "0" ]] \
  || { echo "g2_nq1_disposable already has public tables; reset it explicitly before bootstrapping again" >&2; exit 3; }
...
# 4. T source: exact accepted R head, shared dependency tree, the full tracked history (S1..R).
...
[[ -e "$G2_NQ1_OLD_ROOT/node_modules" ]] || ln -s "$ROOT/node_modules" "$G2_NQ1_OLD_ROOT/node_modules"
...
# N/Q1 ships no migration: the candidate's migration directories are byte-identical to the T root's.
...
if [[ "$MODE" == bootstrap ]]; then
# 5. Full accepted history (S1..R) through the real release mechanism from the T root.
#    Runs as the non-superuser BYPASSRLS `postgres` role (owner), never as the cluster superuser.
(cd "$G2_NQ1_OLD_ROOT" && DATABASE_URL="$MIGRATE_AUTH_URL" DIRECT_URL="$MIGRATE_AUTH_URL" \
...
# 6. Independent T client generated from the T schema (accepted R head) into its own directory.
mkdir -p "$G2_NQ1_OLD_CLIENT"
PINNED_CLIENT_PKG="$(cd "$ROOT" && node -p 'require("fs").realpathSync(require.resolve("@prisma/client/package.json"))')"
RESOLVED_CLIENT_PKG="$(node -p 'try { require("fs").realpathSync(require.resolve("@prisma/client/package.json", { paths: [process.argv[1]] })) } catch (e) { "UNRESOLVED" }' "$G2_NQ1_OLD_CLIENT")"
[[ "$RESOLVED_CLIENT_PKG" == "$PINNED_CLIENT_PKG" ]] \
  || { echo "@prisma/client does not resolve from $G2_NQ1_OLD_CLIENT to the pinned tree ($RESOLVED_CLIENT_PKG != $PINNED_CLIENT_PKG); refusing to generate" >&2; exit 6; }
...
OLD_SCHEMA_COPY="$G2_NQ1_OLD_CLIENT/.schema-for-generate.prisma"
sed "s|provider = \"prisma-client-js\"|provider = \"prisma-client-js\"\n  output   = \"$G2_NQ1_OLD_CLIENT\"|" \
  "$G2_NQ1_OLD_ROOT/prisma/schema.prisma" > "$OLD_SCHEMA_COPY"
(cd "$G2_NQ1_OLD_ROOT" && "$ROOT/node_modules/.bin/prisma" generate --schema "$OLD_SCHEMA_COPY" >/dev/null)
grep -q 'model ScoutReconstructionLedger' "$G2_NQ1_OLD_CLIENT/schema.prisma"
O_ENGINE_SHA="$(hash_required 'generated T client engine' "$G2_NQ1_OLD_CLIENT/$ENGINE")"
[[ "$O_ENGINE_SHA" == "$PINNED_ENGINE_SHA" ]] \
  || { echo "generated T client engine differs from the pinned @prisma/engines copy" >&2; exit 6; }
# The T client's copied runtime must be the pinned package runtime: identical bytes, or the pinned
...
O_RUNTIME="$G2_NQ1_OLD_CLIENT/runtime/library.js"
O_RUNTIME_SHA="$(hash_required 'generated T client runtime' "$O_RUNTIME")"
...
[[ "$O_RUNTIME_SIZE" -ge "$PINNED_RUNTIME_SIZE" ]] && cmp -s <(tail -c "$PINNED_RUNTIME_SIZE" "$O_RUNTIME") "$PINNED_RUNTIME" \
  || { echo "generated T client runtime ($O_RUNTIME_SIZE B, $O_RUNTIME_SHA) does not end with the pinned @prisma/client runtime ($PINNED_RUNTIME_SIZE B, $PINNED_RUNTIME_SHA)" >&2; exit 6; }
echo "O_CLIENT_GENERATED dir=$G2_NQ1_OLD_CLIENT engine_sha256=$O_ENGINE_SHA runtime=$O_RUNTIME runtime_sha256=$O_RUNTIME_SHA runtime_header_bytes=$((O_RUNTIME_SIZE - PINNED_RUNTIME_SIZE)) pinned_runtime=$PINNED_RUNTIME pinned_runtime_sha256=$PINNED_RUNTIME_SHA"
# The T client is R's: optional ledger provenance (`String?`, R's D1) and both wide keys declared.
awk '/model ScoutReconstructionLedger \{/,/\}/' "$G2_NQ1_OLD_CLIENT/schema.prisma" | grep -Eq 'source_platform +String\?' \
  || { echo "generated T client does not carry R's optional ledger source_platform" >&2; exit 6; }
for key in ScoutIngestEntity_identity_key ScoutReconstructionLedger_identity_key; do
  grep -q "map: \"$key\"" "$G2_NQ1_OLD_CLIENT/schema.prisma" \
    || { echo "generated T client schema lacks $key; it is not the R client" >&2; exit 6; }
done

# 7. Candidate (N) client in the candidate root: the N client generated once, before commit, in
#    the isolated copy of the accepted R dependency tree (N changes prisma/schema.prisma: the ledger
#    provenance becomes required) is only VERIFIED here; no `prisma generate` runs in the bootstrap.
...
# The generated client must be N's: REQUIRED ledger provenance (`String`, not `String?`) and both
# wide keys declared (inherited from R), not a stale R/B client.
...
! awk '/model ScoutReconstructionLedger \{/,/\}/' "$ROOT/node_modules/.prisma/client/schema.prisma" | grep -Eq 'source_platform +String\?' \
  || { echo "candidate client still carries optional ledger source_platform; the N client was not generated" >&2; exit 7; }
for key in ScoutIngestEntity_identity_key ScoutReconstructionLedger_identity_key; do
  grep -q "map: \"$key\"" "$ROOT/node_modules/.prisma/client/schema.prisma" \
    || { echo "candidate client schema lacks $key; the N client was not generated from the R-derived schema" >&2; exit 7; }
done

if [[ "$MODE" == generate-only ]]; then echo "G2_NQ1_GENERATE_OK"; exit 0; fi   # no connection in this mode

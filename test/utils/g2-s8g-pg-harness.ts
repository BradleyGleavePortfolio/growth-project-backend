/**
 * S8-G G2 live-proof helpers: derived by substitution from the accepted S8-C harness
 * test/utils/g2-s8c-pg-harness.ts (at 62471b11), which stays byte-identical. Same real psql, same
 * real generated Prisma client in separate OS processes (test/utils/g2-s8g-worker.cjs drives the
 ...
 * or query mocks; bound to the S8-G-only identity guard test/utils/g2-s8g-db.ts and the G2_S8G_*
 * environment. Only test/rls-g2-s8g.spec.ts (via test/utils/g2-s8g-harness.ts) imports this.
 *
 * There is no OLD side: S8-G ships no migration, so the base 62471b11 prisma tree (identical to 1c5fbb04, through the
...
/** Base pin (kept identical in test/utils/g2-s8g-bootstrap.sh and test/scout/g2-s8g-db-guard.spec.ts). */
export const BASE_HEAD: string = G2_S8G_BASE_HEAD;
...
/** 171 accepted migrations through S8-B plus S7-L; S8-G adds none. */
export const EXPECTED_MIGRATIONS = 172;

const raw = process.env.G2_S8G_DATABASE_URL;
const password = process.env.G2_S8G_PASSWORD;
export const psql = process.env.G2_S8G_PSQL;
export const directory = process.env.G2_S8G_DATA_DIRECTORY;
export const expectedVersion = Number(process.env.G2_S8G_SERVER_VERSION ?? '170006');
if (!raw || !password || !psql || !directory) {
  throw new Error('S8-G G2 proof requires explicit database, password, psql and data directory');
}
export const target = g2S8gTestTarget(raw, process.env.G2_S8G_CONFIRM);
/** The attested candidate head this run is bound to: the runtime root must be exactly there, clean. */
export const candidateHead = g2S8gCandidateHead(
  process.env[G2_S8G_CANDIDATE_HEAD_ENV],
...
export const migrationUrl = asRole(G2_S8G_MIGRATION_ROLE);
/** Cluster superuser: lock-wait observation in pg_stat_activity only. */
const adminUrl = asRole(G2_S8G_ADMIN_ROLE);
 ...
 *  S8-G parsers and injects as the owned family registry; see test/utils/g2-s8g-worker.cjs. */
export function worker(options: Record<string, unknown> = {}) {
  const name = `g2g_${++sequence}`;
  const service = new URL(withFixturePassword(target.prismaUrl, password, G2_S8G_RUNTIME_ROLE));
  ...
  const child = fork(resolve(root, 'test/utils/g2-s8g-worker.cjs'), [], {
    cwd: root,
    execArgv: ['-r', 'ts-node/register/transpile-only'],
    env: {
      ...process.env,
      G2_S8G_WORKER: JSON.stringify(config),
...
export function holdTransaction(statements: string, role: string = G2_S8G_RUNTIME_ROLE) {
...
export function prisma(schemaRoot: string, args: string[]): { ok: boolean; output: string } {
  const url = withFixturePassword(target.prismaUrl, password, G2_S8G_MIGRATION_ROLE);
  try {

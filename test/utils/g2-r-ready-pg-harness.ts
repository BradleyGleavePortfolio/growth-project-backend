/**
 * S7-3' G2 R/ready live-proof helpers: derived by substitution from the accepted B harness
 * test/utils/g2-b-drain-pg-harness.ts, which stays byte-identical. Same real psql, same real
 * generated Prisma clients in separate OS processes (test/utils/g2-tq0-worker.cjs, shared
 * unchanged), no transaction or query mocks; bound to the R-only identity guard
 * test/utils/g2-r-ready-db.ts and the G2_R_* environment. Only test/rls-g2-r-ready.spec.ts
 * (via test/utils/g2-r-ready-harness.ts) imports this.
 */
...
const raw = process.env.G2_R_DATABASE_URL;
const password = process.env.G2_R_PASSWORD;
export const psql = process.env.G2_R_PSQL;
export const oldRoot = process.env.G2_R_OLD_ROOT;
export const oldClient = process.env.G2_R_OLD_CLIENT;
export const directory = process.env.G2_R_DATA_DIRECTORY;
export const expectedVersion = Number(process.env.G2_R_SERVER_VERSION ?? '170006');
if (!raw || !password || !psql || !oldRoot || !oldClient || !directory) {
  throw new Error(
    'R/ready G2 proof requires explicit database, password, psql, data directory, old source and old client',
  );
}
export const target = g2RReadyTestTarget(raw, process.env.G2_R_CONFIRM);
...
export const migrationUrl = asRole(G2_R_MIGRATION_ROLE);
/** Cluster superuser: lock-wait observation in pg_stat_activity only. */
const adminUrl = asRole(G2_R_ADMIN_ROLE);
...
export function worker(options: Record<string, unknown> = {}, old = false) {
  const name = `g2r_${++sequence}`;
  const service = new URL(withFixturePassword(target.prismaUrl, password, G2_R_RUNTIME_ROLE));
...
export function holdTransaction(statements: string, role: string = G2_R_RUNTIME_ROLE) {
...
export const holdAdvisory = (key: number) =>
  holdTransaction(`SELECT pg_advisory_xact_lock(${key})`, G2_R_MIGRATION_ROLE);
...
export function prisma(schemaRoot: string, args: string[]): { ok: boolean; output: string } {
  const url = withFixturePassword(target.prismaUrl, password, G2_R_MIGRATION_ROLE);
  try {

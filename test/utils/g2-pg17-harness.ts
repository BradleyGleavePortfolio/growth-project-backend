...
if (!raw || !password || !psql || !oldRoot || !oldClient || !directory) {
  throw new Error(
    'S5 G2 proof requires explicit database, password, psql, data directory, old source and old client',
  );
}
...
export const holdAdvisory = (key: number) =>
  holdTransaction(`SELECT pg_advisory_xact_lock(${key})`, G2_PG17_MIGRATION_ROLE);

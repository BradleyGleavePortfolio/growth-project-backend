#!/usr/bin/env bash
# S10-B G2 proof bootstrap, derived by substitution from the S9-C bootstrap
# test/utils/g2-s9c-bootstrap.sh (d3a9-s9c-r2; itself from the accepted S8-G bootstrap) for the
# S10-B-only disposable cluster/database. S10-B ships ONE migration
# (20270124000000_scout_run_observation_expand) and the three matching schema.prisma models: the
# candidate's prisma diff against the base a4af8e33 must be exactly prisma/schema.prisma plus that
# migration directory (migration.sql + down.sql). The full accepted history (172 migrations, S8-B
# and S7-L included) plus the S10-B expand = 173 is installed from the CANDIDATE root through the
# real release mechanism (`prisma migrate deploy`), and the spec then drives the candidate
# ObservationService / ScoutLifecycleService / ScoutIngestService against it.
...
#   G2_S10B_DATABASE_URL  postgresql://s10b_super@127.0.0.1:<port>/g2_s10b_disposable?schema=public&connection_limit=2
#   G2_S10B_CONFIRM       g2_s10b_disposable:<port>
#   G2_S10B_PASSWORD      disposable local fixture password shared by s10b_super/postgres/service_role
#                         (never written to a file by this harness; only into process env / in-memory URLs)
#   G2_S10B_PSQL          absolute path of a psql binary compatible with the PG17 server
#   G2_S10B_CANDIDATE_HEAD  the attested final candidate head (40 hex); the runtime root must be
#                        checked out exactly there and clean, and it must not be the base itself
# Optional:
#   G2_S10B_SERVER_VERSION  expected server_version_num (default 170006 = 17.6)
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
# Base pin (kept identical in test/utils/g2-s10b-pg-harness.ts and test/utils/g2-s10b-db.ts).
BASE_HEAD=a4af8e330bd4d6f882f0aebd411200b76d651aba
...
# 172 accepted migrations through S7-L tracked by the base, plus the S10-B expand = 173.
EXPECTED_MIGRATIONS=173
EXPECTED_VERSION="${G2_S10B_SERVER_VERSION:-170006}"
# Distinctive S10-B fixture markers: pinned literals, never read from the environment (see
# test/utils/g2-s10b-db.ts G2_S10B_CLUSTER_MARKER / G2_S10B_DATABASE_MARKER, kept identical by
# test/rls-g2-s10b.spec.ts). The lane's postgresql.conf must set
# cluster_name = 's10b-disposable-pg17'; the disposable database carries DB_MARKER as its comment.
CLUSTER_MARKER=s10b-disposable-pg17
DB_MARKER=s10b-g2-run-observation-synthetic-disposable-fixture-safe-to-drop

for name in G2_S10B_DATABASE_URL G2_S10B_CONFIRM G2_S10B_PASSWORD G2_S10B_PSQL G2_S10B_CANDIDATE_HEAD; do
  [[ -n "${!name:-}" ]] || { echo "missing $name" >&2; exit 2; }
done
# Candidate binding (mirrors g2S10bCandidateHead in test/utils/g2-s10b-db.ts): one attested head, clean.
[[ "$G2_S10B_CANDIDATE_HEAD" =~ ^[0-9a-f]{40}$ ]] || { echo "G2_S10B_CANDIDATE_HEAD is not a 40-hex commit id" >&2; exit 2; }
[[ "$G2_S10B_CANDIDATE_HEAD" != "$BASE_HEAD" ]] || { echo "G2_S10B_CANDIDATE_HEAD is the accepted base, not a candidate" >&2; exit 2; }
ACTUAL_HEAD="$(git -C "$ROOT" rev-parse HEAD)"
[[ "$ACTUAL_HEAD" == "$G2_S10B_CANDIDATE_HEAD" ]] || { echo "runtime root $ROOT is at $ACTUAL_HEAD, not the attested candidate $G2_S10B_CANDIDATE_HEAD" >&2; exit 2; }
...
[[ -x "$G2_S10B_PSQL" ]] || { echo "psql binary not executable: $G2_S10B_PSQL" >&2; exit 2; }
...
eval "$(cd "$ROOT" && node -r ts-node/register/transpile-only -e '
  const { g2S10bTestTarget, withFixturePassword } = require("./test/utils/g2-s10b-db");
  const t = g2S10bTestTarget(process.env.G2_S10B_DATABASE_URL, process.env.G2_S10B_CONFIRM);
  ...
  console.log("MIGRATE_AUTH_URL=" + q(withFixturePassword(t.prismaUrl, process.env.G2_S10B_PASSWORD, "postgres")));
')"
export PGPASSWORD="$G2_S10B_PASSWORD"

psql_maint() { "$G2_S10B_PSQL" -X -w -qAt -v ON_ERROR_STOP=1 "$MAINT_URL" "$@"; }
psql_db() { "$G2_S10B_PSQL" -X -w -qAt -v ON_ERROR_STOP=1 "$PSQL_URL" "$@"; }

MODE="${1:-bootstrap}"
case "$MODE" in bootstrap|verify-only) ;; *) echo "usage: g2-s10b-bootstrap.sh [bootstrap|verify-only]" >&2; exit 2;; esac
...
#     not the S10-B lane initialised for this proof; stop without creating roles or databases.
CLUSTER="$(psql_maint -c "SELECT current_setting('cluster_name')")"
[[ "$CLUSTER" == "$CLUSTER_MARKER" ]] || { echo "cluster_name '$CLUSTER' is not the S10-B fixture marker '$CLUSTER_MARKER'; refusing to touch this server" >&2; exit 3; }
...
psql_maint -v pw="$G2_S10B_PASSWORD" <<'SQL'
DO $$ BEGIN
...
#    by the operator (DROP DATABASE g2_s10b_disposable as the cluster superuser) first.
...
if [[ "$(psql_maint -c "SELECT count(*) FROM pg_database WHERE datname='g2_s10b_disposable'")" == "0" ]]; then
  psql_maint -c 'CREATE DATABASE g2_s10b_disposable OWNER postgres'
  psql_maint -v marker="$DB_MARKER" <<'SQL'
COMMENT ON DATABASE g2_s10b_disposable IS :'marker';
SQL
fi
EXISTING_MARKER="$(psql_maint -c "SELECT COALESCE(shobj_description(oid,'pg_database'),'') FROM pg_database WHERE datname='g2_s10b_disposable'")"
[[ "$EXISTING_MARKER" == "$DB_MARKER" ]] \
  || { echo "g2_s10b_disposable exists without the S10-B disposable marker comment; refusing to reuse it" >&2; exit 3; }
psql_maint -c 'ALTER DATABASE g2_s10b_disposable OWNER TO postgres'
[[ "$(psql_db -c "SELECT count(*) FROM pg_tables WHERE schemaname='public'")" == "0" ]] \
  || { echo "g2_s10b_disposable already has public tables; reset it explicitly before bootstrapping again" >&2; exit 3; }
...
# 4. Candidate source: S10-B ships exactly one migration directory and the schema models. The prisma
#    diff against the base is exactly prisma/schema.prisma + the S10-B migration.sql/down.sql, and
#    the tree tracks exactly the accepted 172 directories plus S10-B, S8-B and S7-L included.
git -C "$ROOT" cat-file -e "$BASE_HEAD^{commit}" || { echo "base $BASE_HEAD unknown in $ROOT" >&2; exit 4; }
PRISMA_DIFF="$(git -C "$ROOT" diff --name-only "$BASE_HEAD" HEAD -- prisma | LC_ALL=C sort | tr '\n' ' ')"
EXPECTED_DIFF="prisma/migrations/$S10B_MIGRATION/down.sql prisma/migrations/$S10B_MIGRATION/migration.sql prisma/schema.prisma "
[[ "$PRISMA_DIFF" == "$EXPECTED_DIFF" ]] \
  || { echo "candidate prisma diff vs base $BASE_HEAD is '$PRISMA_DIFF', expected exactly '$EXPECTED_DIFF'" >&2; exit 4; }
[[ -z "$(git -C "$ROOT" diff --name-only --diff-filter=DMR "$BASE_HEAD" HEAD -- prisma/migrations)" ]] \
  || { echo "candidate modifies, renames or deletes an accepted migration" >&2; exit 4; }
...
[[ -f "$ROOT/prisma/migrations/$S10B_MIGRATION/migration.sql" && -f "$ROOT/prisma/migrations/$S10B_MIGRATION/down.sql" ]] \
  || { echo "candidate lacks the S10-B migration.sql/down.sql" >&2; exit 4; }

if [[ "$MODE" == bootstrap ]]; then
# 5. Full accepted history (S8-B and S7-L included) plus S10-B through the real release mechanism from the candidate root.
...
# The three S10-B tables exist with RLS enabled and forced.
for rel in ScoutRunDeclaration ScoutRunObservation ScoutRunSettledBasis; do
  [[ "$(psql_db -c "SELECT relrowsecurity AND relforcerowsecurity FROM pg_class WHERE oid=to_regclass('public.\"$rel\"')")" == "t" ]] \
    || { echo "$rel absent or without forced RLS after the S10-B expand" >&2; exit 5; }
done
# S7-L run objects the S10-B run lock and refusals depend on must be PRESENT (the accepted target of this slice).
...
# 6. Candidate client in the candidate root: generated once by the runtime setup from the CANDIDATE
#    schema (base + the three S10-B models) and only VERIFIED here; no `prisma generate`.
...
for model in ScoutRunDeclaration ScoutRunObservation ScoutRunSettledBasis; do
  grep -q "model $model " "$ROOT/node_modules/.prisma/client/schema.prisma" \
    || { echo "candidate client lacks $model (a pre-S10-B client; regenerate in the runtime setup)" >&2; exit 7; }
done
...
if [[ "$MODE" == verify-only ]]; then echo "G2_S10B_VERIFY_OK"; exit 0; fi   # no connection in this mode

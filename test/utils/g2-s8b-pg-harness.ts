/**
 * S8-B G2 live-proof helpers: derived by substitution from the committed N/Q1 harness
 * test/utils/g2-nq1-pg-harness.ts (blob c8f6fea8 at 61b93cff), which stays byte-identical. Same
 * real psql, same real generated Prisma clients in separate OS processes (the committed, unchanged
 * test/utils/g2-tq0-worker.cjs drives the real ScoutReconstructService writer and the roster /
 * entities readers), no transaction or query mocks; bound to the S8-B-only identity guard
 * test/utils/g2-s8b-db.ts and the G2_S8B_* environment. Only test/rls-g2-s8b.spec.ts (via
 * test/utils/g2-s8b-harness.ts) imports this.
 *
 * The "old" process role here is the S8-B base head (OLD_HEAD: the accepted C head 1b6cc661)
 * with the OLD client generated in
 * that detached checkout, i.e. an image that knows nothing about ImportNativeProvenance or the
 * ledger target_kind column. `worker(..., true)` runs it. The barrier/cursor helpers of the N/Q1
 * harness are deliberately not carried: S8-B proves the provenance expand only.
 */
...
/** OLD side pin (kept identical in test/utils/g2-s8b-bootstrap.sh and g2-s8b-old-root.sh). */
export const OLD_HEAD = '1b6cc66164b3398d573ba92f0c8f6b039494be24';
export const S8B_MIGRATION = '20270122000000_scout_native_provenance_expand';

const raw = process.env.G2_S8B_DATABASE_URL;
const password = process.env.G2_S8B_PASSWORD;
export const psql = process.env.G2_S8B_PSQL;
export const oldRoot = process.env.G2_S8B_OLD_ROOT;
export const oldClient = process.env.G2_S8B_OLD_CLIENT;
export const directory = process.env.G2_S8B_DATA_DIRECTORY;
export const expectedVersion = Number(process.env.G2_S8B_SERVER_VERSION ?? '170006');
if (!raw || !password || !psql || !oldRoot || !oldClient || !directory) {
  throw new Error(
    'S8-B G2 proof requires explicit database, password, psql, data directory, OLD source and OLD client',
  );
}
export const target = g2S8bTestTarget(raw, process.env.G2_S8B_CONFIRM);
...
export const migrationUrl = asRole(G2_S8B_MIGRATION_ROLE);
/** Cluster superuser: lock-wait observation in pg_stat_activity only. */
const adminUrl = asRole(G2_S8B_ADMIN_ROLE);
...
/** Operator form: `psql -v ON_ERROR_STOP=1 --single-transaction -f <file>`. S8-B's files carry
...
export const stageSql = resolve(root, 'prisma/migrations', S8B_MIGRATION);
...
let sequence = 0;
/** One real service call in an independent OS process with the OLD (pre-S8-B) or the candidate
 *  generated client. `options.action` ∈ reconstruct (default; the real N ledger writer for
 *  `options.family`) | `roster` | `entities`; see the unchanged test/utils/g2-tq0-worker.cjs. */
export function worker(options: Record<string, unknown> = {}, old = false) {
  const name = `g2b_${++sequence}`;
  const service = new URL(withFixturePassword(target.prismaUrl, password, G2_S8B_RUNTIME_ROLE));
...
export function holdTransaction(statements: string, role: string = G2_S8B_RUNTIME_ROLE) {
...
export function prisma(schemaRoot: string, args: string[]): { ok: boolean; output: string } {
  const url = withFixturePassword(target.prismaUrl, password, G2_S8B_MIGRATION_ROLE);
  try {

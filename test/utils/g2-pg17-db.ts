...
export const G2_PG17_LOGIN_ROLES = new Set([
  G2_PG17_ADMIN_ROLE,
  G2_PG17_MIGRATION_ROLE,
  G2_PG17_RUNTIME_ROLE,
]);
...
export function g2Pg17TestTarget(raw: string, confirmation?: string) {
  ...
  if (
    ...
    throw new Error(
      'S5 G2 proof requires its explicitly confirmed loopback disposable database and port',
    );
  }
...
export function withFixturePassword(
  ...
  username = G2_PG17_ROLE,
): string {
  if (!password || /[\s@/:?#]/.test(password))
    throw new Error('S5 G2 proof requires a plain fixture password in the environment');
  if (!G2_PG17_LOGIN_ROLES.has(username))
    throw new Error('S5 G2 proof connects only as a fixture matrix login role');

/**
 * G2-S9-C live-proof observations layered on the S9-C-only derivative
 * (test/utils/g2-s9c-pg-harness.ts) of the accepted S8-G harness (derived from test/utils/g2-s8g-harness.ts at 771db62a). Union of the S7-L run-lifecycle
 ...
 * worker injects, catalog rows, target/provenance/ledger snapshots). Adds only what S8-G needed (S9-C adds nothing):
 ...
 * ScoutLifecycleService → ScoutReconstructService in test/utils/g2-s9c-worker.cjs. Imported only
 * by the explicitly guarded test/rls-g2-s9c.spec.ts.
 */
import { json, quote, sql } from './g2-s9c-pg-harness';
...
export const PLATFORM = 's9c-proof';

/**
 * S9-C G2 live-proof helpers: derived by substitution from the accepted S8-G harness
 * test/utils/g2-s8g-pg-harness.ts (at 771db62a), which stays byte-identical. Same real psql, same
 * real generated Prisma client in separate OS processes (test/utils/g2-s9c-worker.cjs drives the
 * real ScoutService → ScoutLifecycleService → ScoutReconstructService settle path with the owned
 * family registry injected from an explicit fixture source spec + native rule set), no transaction
 * or query mocks; bound to the S9-C-only identity guard test/utils/g2-s9c-db.ts and the G2_S9C_*
 * environment. Only test/rls-g2-s9c.spec.ts (via test/utils/g2-s9c-harness.ts) imports this.
 *
 * There is no OLD side: S9-C ships no migration, so the base 5407efae prisma tree (S9-B merge; identical to 1c5fbb04, through the
 * accepted S7-L migration) is the only schema and the accepted S7-L run objects plus the S8-B
 * native provenance objects are the proof target. No up/down file helpers are carried.
 */
...
/** Base pin (kept identical in test/utils/g2-s9c-bootstrap.sh and test/scout/g2-s9c-db-guard.spec.ts). */
export const BASE_HEAD: string = G2_S9C_BASE_HEAD;
...
/** 171 accepted migrations through S8-B plus S7-L = 172; S9-C adds none. */
export const EXPECTED_MIGRATIONS = 172;

const raw = process.env.G2_S9C_DATABASE_URL;
const password = process.env.G2_S9C_PASSWORD;
export const psql = process.env.G2_S9C_PSQL;
export const directory = process.env.G2_S9C_DATA_DIRECTORY;
export const expectedVersion = Number(process.env.G2_S9C_SERVER_VERSION ?? '170006');
if (!raw || !password || !psql || !directory) {
  throw new Error('S9-C G2 proof requires explicit database, password, psql and data directory');
}
export const target = g2S9cTestTarget(raw, process.env.G2_S9C_CONFIRM);
/** The attested candidate head this run is bound to: the runtime root must be exactly there, clean. */
export const candidateHead = g2S9cCandidateHead(
  process.env[G2_S9C_CANDIDATE_HEAD_ENV],
...
export const migrationUrl = asRole(G2_S9C_MIGRATION_ROLE);
/** Cluster superuser: lock-wait observation in pg_stat_activity only. */
const adminUrl = asRole(G2_S9C_ADMIN_ROLE);
 ...
 *  test/utils/g2-s9c-worker.cjs. */
export function worker(options: Record<string, unknown> = {}) {
  const name = `g2g_${++sequence}`;
  const service = new URL(withFixturePassword(target.prismaUrl, password, G2_S9C_RUNTIME_ROLE));
  ...
  const child = fork(resolve(root, 'test/utils/g2-s9c-worker.cjs'), [], {
    cwd: root,
    execArgv: ['-r', 'ts-node/register/transpile-only'],
    env: {
      ...process.env,
      G2_S9C_WORKER: JSON.stringify(config),
...
export function holdTransaction(statements: string, role: string = G2_S9C_RUNTIME_ROLE) {
...
export function prisma(schemaRoot: string, args: string[]): { ok: boolean; output: string } {
  const url = withFixturePassword(target.prismaUrl, password, G2_S9C_MIGRATION_ROLE);
  try {

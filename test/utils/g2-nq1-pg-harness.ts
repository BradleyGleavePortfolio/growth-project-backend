/**
 * S7-3' G2 N/Q1 live-proof helpers: derived by substitution from the accepted R harness
 * test/utils/g2-r-ready-pg-harness.ts, which stays byte-identical. Same real psql, same real
 * generated Prisma clients in separate OS processes (test/utils/g2-tq0-worker.cjs, shared
 * unchanged), no transaction or query mocks; bound to the N/Q1-only identity guard
 * test/utils/g2-nq1-db.ts and the G2_NQ1_* environment. Only test/rls-g2-nq1.spec.ts
 * (via test/utils/g2-nq1-harness.ts) imports this.
 *
 * The "old" process role here is T (the accepted R head 7d2895e1: T writer + Q0 readers with
 * the T client generated in that detached checkout), not O. `worker(..., true)` runs T.
 */
...
export const OLD_HEAD = '7d2895e1fe03ea82353e8ce0b07aacaf66af74c8';
export const E_MIGRATION = '20270118000000_scout_ledger_platform_expand';

const raw = process.env.G2_NQ1_DATABASE_URL;
const password = process.env.G2_NQ1_PASSWORD;
export const psql = process.env.G2_NQ1_PSQL;
export const oldRoot = process.env.G2_NQ1_OLD_ROOT;
export const oldClient = process.env.G2_NQ1_OLD_CLIENT;
export const directory = process.env.G2_NQ1_DATA_DIRECTORY;
export const expectedVersion = Number(process.env.G2_NQ1_SERVER_VERSION ?? '170006');
if (!raw || !password || !psql || !oldRoot || !oldClient || !directory) {
  throw new Error(
    'N/Q1 G2 proof requires explicit database, password, psql, data directory, T source and T client',
  );
}
export const target = g2Nq1TestTarget(raw, process.env.G2_NQ1_CONFIRM);
...
export const migrationUrl = asRole(G2_NQ1_MIGRATION_ROLE);
/** Cluster superuser: lock-wait observation in pg_stat_activity only. */
const adminUrl = asRole(G2_NQ1_ADMIN_ROLE);
...
export function worker(options: Record<string, unknown> = {}, old = false) {
  const name = `g2n_${++sequence}`;
  const service = new URL(withFixturePassword(target.prismaUrl, password, G2_NQ1_RUNTIME_ROLE));
...
export function holdTransaction(statements: string, role: string = G2_NQ1_RUNTIME_ROLE) {
...
export const holdAdvisory = (key: number) =>
  holdTransaction(`SELECT pg_advisory_xact_lock(${key})`, G2_NQ1_MIGRATION_ROLE);
...
export function prisma(schemaRoot: string, args: string[]): { ok: boolean; output: string } {
  const url = withFixturePassword(target.prismaUrl, password, G2_NQ1_MIGRATION_ROLE);
  try {

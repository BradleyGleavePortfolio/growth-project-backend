#!/usr/bin/env bash
# S9-C G2 proof bootstrap, derived by substitution from the accepted S8-C bootstrap
# test/utils/g2-s8g-bootstrap.sh (at 771db62a, unchanged) for the S9-C-only disposable
# cluster/database. S9-C ships NO migration: the candidate's prisma tree must be byte-identical to
# the base 5407efae (S9-B merge; prisma tree identical to 1c5fbb04), so there is no OLD side here. The full accepted history (172 migrations,
...
#   G2_S9C_DATABASE_URL  postgresql://s9c_super@127.0.0.1:<port>/g2_s9c_disposable?schema=public&connection_limit=2
#   G2_S9C_CONFIRM       g2_s9c_disposable:<port>
#   G2_S9C_PASSWORD      disposable local fixture password shared by s9c_super/postgres/service_role
#                         (never written to a file by this harness; only into process env / in-memory URLs)
#   G2_S9C_PSQL          absolute path of a psql binary compatible with the PG17 server
#   G2_S9C_CANDIDATE_HEAD  the attested final candidate head (40 hex); the runtime root must be
#                        checked out exactly there and clean, and it must not be the base itself
# Optional:
#   G2_S9C_SERVER_VERSION  expected server_version_num (default 170006 = 17.6)
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
# Base pin (kept identical in test/utils/g2-s9c-pg-harness.ts and test/scout/g2-s9c-db-guard.spec.ts).
BASE_HEAD=5407efae319fd913e973c87f3be0d49786c4a3e0
S8B_MIGRATION=20270122000000_scout_native_provenance_expand
S7L_MIGRATION=20270123000000_scout_run_lifecycle_expand
# 171 accepted migrations through S8-B plus S7-L = 172 tracked by the base; S9-C adds none.
EXPECTED_MIGRATIONS=172
EXPECTED_VERSION="${G2_S9C_SERVER_VERSION:-170006}"
# Distinctive S9-C fixture markers: pinned literals, never read from the environment (see
# test/utils/g2-s9c-db.ts G2_S9C_CLUSTER_MARKER / G2_S9C_DATABASE_MARKER, kept identical by
# test/scout/g2-s9c-db-guard.spec.ts). The lane's postgresql.conf must set
# cluster_name = 's9c-disposable-pg17'; the disposable database carries DB_MARKER as its comment.
CLUSTER_MARKER=s9c-disposable-pg17
DB_MARKER=s9c-g2-reconciliation-wiring-synthetic-disposable-fixture-safe-to-drop

for name in G2_S9C_DATABASE_URL G2_S9C_CONFIRM G2_S9C_PASSWORD G2_S9C_PSQL G2_S9C_CANDIDATE_HEAD; do
  [[ -n "${!name:-}" ]] || { echo "missing $name" >&2; exit 2; }
done
# Candidate binding (mirrors g2S9cCandidateHead in test/utils/g2-s9c-db.ts): one attested head, clean.
[[ "$G2_S9C_CANDIDATE_HEAD" =~ ^[0-9a-f]{40}$ ]] || { echo "G2_S9C_CANDIDATE_HEAD is not a 40-hex commit id" >&2; exit 2; }
[[ "$G2_S9C_CANDIDATE_HEAD" != "$BASE_HEAD" ]] || { echo "G2_S9C_CANDIDATE_HEAD is the accepted base, not a candidate" >&2; exit 2; }
ACTUAL_HEAD="$(git -C "$ROOT" rev-parse HEAD)"
[[ "$ACTUAL_HEAD" == "$G2_S9C_CANDIDATE_HEAD" ]] || { echo "runtime root $ROOT is at $ACTUAL_HEAD, not the attested candidate $G2_S9C_CANDIDATE_HEAD" >&2; exit 2; }
...
[[ -x "$G2_S9C_PSQL" ]] || { echo "psql binary not executable: $G2_S9C_PSQL" >&2; exit 2; }
...
eval "$(cd "$ROOT" && node -r ts-node/register/transpile-only -e '
  const { g2S9cTestTarget, withFixturePassword } = require("./test/utils/g2-s9c-db");
  const t = g2S9cTestTarget(process.env.G2_S9C_DATABASE_URL, process.env.G2_S9C_CONFIRM);
  ...
  console.log("MIGRATE_AUTH_URL=" + q(withFixturePassword(t.prismaUrl, process.env.G2_S9C_PASSWORD, "postgres")));
')"
export PGPASSWORD="$G2_S9C_PASSWORD"

psql_maint() { "$G2_S9C_PSQL" -X -w -qAt -v ON_ERROR_STOP=1 "$MAINT_URL" "$@"; }
psql_db() { "$G2_S9C_PSQL" -X -w -qAt -v ON_ERROR_STOP=1 "$PSQL_URL" "$@"; }

MODE="${1:-bootstrap}"
case "$MODE" in bootstrap|verify-only) ;; *) echo "usage: g2-s9c-bootstrap.sh [bootstrap|verify-only]" >&2; exit 2;; esac
...
#     not the S9-C lane initialised for this proof; stop without creating roles or databases.
CLUSTER="$(psql_maint -c "SELECT current_setting('cluster_name')")"
[[ "$CLUSTER" == "$CLUSTER_MARKER" ]] || { echo "cluster_name '$CLUSTER' is not the S9-C fixture marker '$CLUSTER_MARKER'; refusing to touch this server" >&2; exit 3; }
...
psql_maint -v pw="$G2_S9C_PASSWORD" <<'SQL'
DO $$ BEGIN
...
#    by the operator (DROP DATABASE g2_s9c_disposable as the cluster superuser) first.
...
if [[ "$(psql_maint -c "SELECT count(*) FROM pg_database WHERE datname='g2_s9c_disposable'")" == "0" ]]; then
  psql_maint -c 'CREATE DATABASE g2_s9c_disposable OWNER postgres'
  psql_maint -v marker="$DB_MARKER" <<'SQL'
COMMENT ON DATABASE g2_s9c_disposable IS :'marker';
SQL
fi
EXISTING_MARKER="$(psql_maint -c "SELECT COALESCE(shobj_description(oid,'pg_database'),'') FROM pg_database WHERE datname='g2_s9c_disposable'")"
[[ "$EXISTING_MARKER" == "$DB_MARKER" ]] \
  || { echo "g2_s9c_disposable exists without the S9-C disposable marker comment; refusing to reuse it" >&2; exit 3; }
psql_maint -c 'ALTER DATABASE g2_s9c_disposable OWNER TO postgres'
[[ "$(psql_db -c "SELECT count(*) FROM pg_tables WHERE schemaname='public'")" == "0" ]] \
  || { echo "g2_s9c_disposable already has public tables; reset it explicitly before bootstrapping again" >&2; exit 3; }
...
# 4. Candidate source: S9-C ships no migration. The prisma tree (schema + migrations) must be
...
[[ -z "$(git -C "$ROOT" diff --name-only "$BASE_HEAD" HEAD -- prisma)" ]] \
  || { echo "candidate prisma tree differs from base $BASE_HEAD; S9-C must ship no schema/migration change" >&2; exit 4; }
...
# S7-L run objects the S8-G/S9-C settle path depends on must be PRESENT (the accepted target of this slice).
...
if [[ "$MODE" == verify-only ]]; then echo "G2_S9C_VERIFY_OK"; exit 0; fi   # no connection in this mode

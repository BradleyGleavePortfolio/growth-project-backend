...
const sql = (text: string): string =>
  execFileSync(psql!, ['-X', '-w', '-qAt', '-v', 'ON_ERROR_STOP=1', target.psqlUrl], {
    input: text,
    encoding: 'utf8',
    timeout: 40000,
...
const records = () =>
  json(`SELECT COALESCE(jsonb_agg(to_jsonb(l) ORDER BY source_id,id),'[]')
...
const v2 = (family: string, source = 'a', platform = 'truecoach') =>
  `v2.${encoded(
    JSON.stringify({
      v: 2,
      c: 'coach',
...
beforeAll(() => {
  ...
  expect(readFileSync(resolve(oldRoot!, 'src/scout/scout-reconstruct.service.ts'), 'utf8')).toBe(
    execFileSync(
      'git',
      ['show', '925780e0a1906593e5383c618311b6b17364b8dc:src/scout/scout-reconstruct.service.ts'],
...
describe('deterministically coordinated T/T and O/T processes', () => {
  ...
  it('two absent skip inserts observe real P2002 contention and bounded full-transaction retry', async () => {
    ...
    try {
      ...
      holder.kill();
      sql(
        `DROP TRIGGER g2t_insert_barrier ON "ScoutReconstructionLedger"; DROP FUNCTION g2t_insert_barrier()`,
      );
    }
  });
...
describe('Q0 real scoped reads and unchanged emission', () => {
  ...
  it.each(['clients', 'workouts'])(
    ...
    async (family) => {
        ...
        const next =
          id === 'c'
            ...
            : family === 'clients'
              ? encoded(id)
              : encoded(
                  JSON.stringify({ c: 'coach', i: 'intent', f: family, o: 'source_id:asc', s: id }),
                );
      ...
      expect(cursor(composite.result)).toBe(
        family === 'clients'
          ? encoded('b')
          : encoded(
              JSON.stringify({ c: 'coach', i: 'intent', f: family, o: 'source_id:asc', s: 'b' }),
            ),
      );
      ...
      if (family === 'clients') {
        ...
        sql(
          `UPDATE "ScoutReconstructedEntity" SET entity_type='client_history' WHERE id=${quote(ids[0])}`,
        );
        expect(visible((await run({ action, family, limit: 200 })).result)).toEqual([]);
      }
    },

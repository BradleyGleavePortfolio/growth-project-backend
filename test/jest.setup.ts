...
// AUDIT_LOG_ERASURE_HMAC_SECRET — erasure-token.ts fails fast at import if this
// is missing (GDPR Art. 17 tokens must be keyed). Provide a default so suites
// that import the audit-log substrate boot; the erasure-token spec deletes it
// explicitly to exercise the fail-fast path.
process.env.AUDIT_LOG_ERASURE_HMAC_SECRET =
  process.env.AUDIT_LOG_ERASURE_HMAC_SECRET || 'test-audit-log-erasure-hmac-secret';

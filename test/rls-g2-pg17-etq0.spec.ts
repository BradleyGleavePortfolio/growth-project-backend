...
const nextLegacy = (family: string, s: string) =>
  family === 'clients' ? encoded(s) : legacyEntityCursor(family, s);

// Teardown authority (S5-R3-A-01). Jest runs afterAll even when beforeAll throws, so an identity,
// marker or pre-state refusal in beforeAll must NOT be followed by mutating cleanup against a
// fixture this proof never accepted. The flag is set only after every read-only gate below has
// passed and immediately before the first intended setup mutation, so an authorized but partial
// setup (a failed GRANT/ALTER/reset) still cleans up, while a refused setup issues zero DDL/DML.
...
beforeAll(() => {
  ...
  expect(identity).toMatchObject({
    database: 'g2_s5_etq0_disposable',
    address: '127.0.0.1',
    port: target.port,
    directory,
    user: 'postgres',
  ...
  expect(
    sql(`SELECT count(*) FROM pg_tables WHERE schemaname='public' AND tableowner<>'postgres'`),
  ).toBe('0');
  ...
  expect(sql(`SELECT count(*) FROM "_prisma_migrations" WHERE migration_name=${quote(E)}`)).toBe(
    '0',
  );
  ...
  for (const file of [
    'scout-reconstruct.service.ts',
    'scout-roster.service.ts',
    'scout-entities.service.ts',
  ]) {
    expect(readFileSync(resolve(oldRoot!, 'src/scout', file), 'utf8')).toBe(
      gitShow(`${OLD_HEAD}:src/scout/${file}`),
    );
  }
  expect(
    sql(`SELECT rolbypassrls||':'||rolcanlogin FROM pg_roles WHERE rolname='service_role'`),
  ).toBe('true:true');
  expect(
    sql(`SELECT string_agg(rolsuper::text||':'||rolbypassrls::text,',' ORDER BY rolname)
    FROM pg_roles WHERE rolname IN ('anon','authenticated')`),
  ).toBe('false:false,false:false');
  // Every statement above is a read-only observation (SELECT / file read / git show). From here
  // on setup mutates the accepted fixture; authorize teardown first so partial setup cleans up.
  ...
  console.warn(
    'PG17_API_ROLE_TABLE_PRIVILEGES',
    sql(`SELECT string_agg(table_name||'='||privileges,' ' ORDER BY table_name) FROM (
...
afterAll(() => {
  if (!teardownAuthorized) {
    // Setup was refused before its first mutation: nothing of ours exists on this target, and the
    // target's identity was not accepted. Record the skip; issue no DDL/DML at all.
    console.warn(
...
describe('stage 1: E on a populated narrow base with the actual O binary', () => {
  ...
  const _families = ['clients', 'workouts', 'client_history'];
  let before: any[];
  ...
  const readAll = async (family: string) => {
    ...
    for (let n = 0; n < 10; n++) {
      const page = (await run({ action: actionOf(family), family, cursor: after, limit: 7 }, true))
        .result;
      pages.push(page);
  ...
  it('populates synthetic legacy history without a provenance column and O reconstructs 600 staged rows', async () => {
    ...
    expect(
      sql(`SELECT count(DISTINCT entity_type) FROM "ScoutReconstructionLedger"
      WHERE coach_id='coach' AND intent_id='intent' AND source_id='h00001'`),
    ).toBe('3');
    ...
    expect(
      sql(`SELECT count(*) FROM "ScoutReconstructionLedger" WHERE coach_id='coach' AND intent_id='intent'
      AND entity_type='clients' AND source_id LIKE 's%' GROUP BY status ORDER BY status`),
    ).toBe('12\n588');
  ...
  it('applies E: rows, OIDs, keys, RLS and policies preserved; column nullable without default', () => {
    ...
    expect(
      allLedger().map((r: any) => {
        const { source_platform: _source_platform, ...rest } = r;
        return rest;
      }),
    ).toEqual(before);
    expect(
      sql('SELECT count(*) FROM "ScoutReconstructionLedger" WHERE source_platform IS NOT NULL'),
    ).toBe('0');
    ...
    expect(
      sql(`SELECT count(*) FROM pg_index WHERE indexrelid IN (
      'public."ScoutIngestEntity_coach_id_intent_id_source_id_key"'::regclass,
      'public.${ledgerKey}'::regclass) AND indisunique AND indisvalid`),
    ).toBe('2');
  ...
  it('keeps the actual O writer and readers working on E, creating NULL provenance', async () => {
    ...
    expect(
      sql(`SELECT count(*) FROM "ScoutReconstructionLedger" WHERE entity_type='workouts'
      AND source_id LIKE 'w%' AND source_platform IS NULL`),
    ).toBe('30');
    expect(
      o.queries
        .filter((q) => q.startsWith('INSERT INTO "public"."ScoutReconstructionLedger"'))
        .every((q) => !q.includes('source_platform')),
    ).toBe(true);
    // O replay on E is still idempotent and its page enumeration is byte-identical to pre-E.
    expect((await run({}, true)).result).toEqual({
      intent_id: 'intent',
      staged: 600,
      reconstructed: 596,
      skipped: 8,
    ...
    const stable = (pages: any[]) =>
      pages.map((p) => ({
        ...p,
        persons: p.persons.map(({ updated_at: _updated_at, ...rest }: any) => rest),
      }));
    expect(stable(replayPages)).toEqual(stable(oPages.clients));
    expect(replayPages.map((p) => p.page.next_cursor)).toEqual(
      oPages.clients.map((p: any) => p.page.next_cursor),
    );
  ...
  it('retains forced RLS denial for anon/authenticated on every touched table, even with a hostile claim', () => {
    ...
    for (const role of ['anon', 'authenticated']) {
      for (const table of [
        ...
        expect(
          last(
            sql(`SET ROLE ${role}; ${claims} SELECT count(*) FROM public."${table}"`).split('\n'),
          ),
        ).toBe('0');
        expect(
          last(
            sql(`SET ROLE ${role}; WITH changed AS (UPDATE public."${table}" SET coach_id='stolen' RETURNING id)
      ...
      refused(
        `SET ROLE ${role}; INSERT INTO public."ScoutIngestEntity" (id,coach_id,intent_id,entity_type,source_id,source_platform,payload)
        VALUES ('bad','coach','intent','clients','bad','truecoach','{}')`,
        'row-level security',
      );
    }
    ...
    expect(
      last(
        sql(`SET ROLE service_role; SELECT count(*) FROM public."ScoutReconstructionLedger"`).split(
          '\n',
        ),
      ),
    ).toBe('1230');
  ...
  it.each(['up', 'down'])(
    'refuses a wrong public index owner in %s on the populated base',
    (direction) => {
      if (direction === 'up') {
        sql(down);
        expect(hasColumn()).toBe('0');
  ...
  it('schema-qualifies both directions despite search_path decoys, leaving 1230 populated rows intact', () => {
    ...
    try {
      ...
      sql(`SET search_path=g2p_shadow,public;\n${up}`);
      expect(hasColumn()).toBe('1');
      expect(sql(`SELECT source_platform FROM g2p_shadow."ScoutReconstructionLedger"`)).toBe(
        'untouched',
      );
  ...
  it.each([true, false])(
    ...
    (populated) => {
      ...
      if (populated)
        sql(
          `UPDATE "ScoutReconstructionLedger" SET source_platform='truecoach' WHERE coach_id='c4' AND source_id='h00001' AND entity_type='clients'`,
        );
      ...
      const claimed = sql(
        `SELECT count(*) FROM "ScoutReconstructionLedger" WHERE source_platform IS NOT NULL`,
      );
      expect(claimed).toBe(populated ? '2' : '0');
      try {
        expect(
          sql(`SELECT rolsuper||':'||rolbypassrls FROM pg_roles WHERE rolname='g2p_ledger_owner'`),
        ).toBe('false:false');
        // psql -q suppresses the SET tag; forced RLS hides every row from the non-bypass owner.
        expect(
          sql(`SET ROLE g2p_ledger_owner; SELECT count(*) FROM public."ScoutReconstructionLedger"`),
        ).toBe('0');
  ...
  it('rolls E back only while every provenance is NULL, preserving rows, then re-applies', () => {
    const rows = allLedger().map((r: any) => {
      const { source_platform: _source_platform, ...rest } = r;
      return rest;
    });
    sql(down);
...
describe('stage 2: E recorded through the real release mechanism', () => {
  it('prisma migrate deploy applies exactly E on the populated database and records history', () => {
    ...
    expect(hasColumn()).toBe('1');
    expect(
      sql(`SELECT count(*) FROM "_prisma_migrations" WHERE migration_name=${quote(E)}
...
describe('stage 3: T provenance, atomicity, accounting and collisions on PostgreSQL 17', () => {
  ...
  it('claims historical NULL history at volume with honest multi-page accounting and identical replay', async () => {
    ...
    expect(
      sql(`SELECT count(*) FROM "ScoutReconstructionLedger" WHERE source_platform='truecoach'`),
    ).toBe('1050');
    expect(
      sql(`SELECT count(*) FROM "ScoutReconstructionLedger" WHERE source_platform IS NULL`),
    ).toBe('0');
    expect(
      sql(
        `SELECT count(*) FROM "ScoutReconstructionLedger" WHERE id LIKE 'legacy-%' AND status='reconstructed'`,
      ),
    ).toBe('297');
    expect(
      sql(
        `SELECT count(*) FROM "ScoutReconstructionLedger" WHERE id LIKE 'legacy-%' AND status='failed'`,
      ),
    ).toBe('3');
  ...
  it('claims only the exact coach/intent/family identity and leaves adjacent tenants untouched', async () => {
    ...
    const adjacent = () =>
      allLedger().filter(
        (r: any) =>
          !(r.coach_id === 'coach' && r.intent_id === 'intent' && r.entity_type === 'clients'),
      );
  ...
  it('creates every outcome with actual provenance, isolates failures and replays honestly', async () => {
    stage('a');
    stage('b', 'clients', 'auto:coachrx.example.com');
    stage('c', 'clients', 'truecoach', 'FAIL');
    stage('d');
    const first = await run();
    expect(first.failure).toBeUndefined();
    expect(first.result).toEqual({
    ...
    expect(records().map((r: any) => [r.source_id, r.status, r.source_platform])).toEqual([
      ['a', 'reconstructed', 'truecoach'],
      ['b', 'skipped', 'auto:coachrx.example.com'],
      ['c', 'failed', 'truecoach'],
      ['d', 'reconstructed', 'truecoach'],
    ]);
  ...
  it.each(['reconstructed', 'skipped', 'failed'])(
    'claims historical NULL on %s attempts',
    async (status) => {
  ...
  it.each(['clients', 'workouts'])(
    'rolls back new and pre-existing %s target changes on late mismatch',
    async (family) => {
      stage('a', family);
  ...
  it.each(['skip', 'throw', 'database'])(
    'preserves success and reason on %s failure/skip, but still checks mismatch',
    async (mode) => {
      stage();
      await run();
      ...
      expect((await run(options)).result).toMatchObject({
        reconstructed: 1,
        failed: 0,
        skipped: 0,
      });
      expect(records()[0]).toMatchObject({
        ...
        reason: 'retained-success',
        source_platform: 'truecoach',
      });
      sql(`UPDATE "ScoutReconstructionLedger" SET source_platform='different'`);
      const previous = records();
      expect((await run(options)).failure).toEqual({
        status: 409,
        message: 'reconstruction provenance conflict',
      });
      expect(records()).toEqual(previous);
  ...
  it('rejects invalid staging before writes and preserves narrow staging/ledger identities', async () => {
    ...
    expect((await run()).failure).toEqual({
      status: 409,
      message: 'reconstruction provenance conflict',
    });
  ...
  it('cross-intent same source: same platform converges on one Person, different platform mints a second', async () => {
    ...
    expect(persons.map((p: any) => p.source_platform).sort()).toEqual([
      'conformance_alpha',
      'truecoach',
    ]);
    ...
    expect(allLedger().map((r: any) => `${r.intent_id}:${r.source_platform}`)).toEqual([
      'i2:truecoach',
      'i3:conformance_alpha',
      'intent:truecoach',
    ]);
  ...
  it('characterizes the retained narrow ledger collision: one intent cannot carry two platforms for one source', async () => {
    ...
    expect(() =>
      ...
      VALUES ('dup','coach','intent','clients','a','conformance_alpha','skipped')`),
    ).toThrow(/ScoutReconstructionLedger_coach_id_intent_id_entity_type_source/);
    // EXPLICIT LIMITATION carried to R/N/C: wide identities are not permitted on E/T.
    expect(sql(`SELECT pg_get_indexdef('public.${ledgerKey}'::regclass)`)).toBe(
      `CREATE UNIQUE INDEX ${ledgerKey} ON public."ScoutReconstructionLedger" USING btree (coach_id, intent_id, entity_type, source_id)`,
    );
  });
...
describe('stage 3b: deterministically coordinated T/T and O/T processes on PostgreSQL 17', () => {
  ...
  it.each([false, true])(
    'concurrent success replays with %s old winner converge on one target',
    async (old) => {
      stage();
  ...
  it('two absent skip inserts observe real P2002 contention and bounded full-transaction retry', async () => {
    ...
    try {
      ...
      expect(results.map((r) => r.queries.filter((q) => q === 'ROLLBACK').length).sort()).toEqual([
        0, 1,
      ]);
      expect(results.map((r) => r.queries.filter((q) => q === 'BEGIN').length).sort()).toEqual([
        1, 2,
      ]);
    } finally {
      one.stop();
      ...
      holder.kill();
      sql(
        `DROP TRIGGER g2p_insert_barrier ON "ScoutReconstructionLedger"; DROP FUNCTION g2p_insert_barrier()`,
      );
    }
  });

  it.each(['skip', 'throw', 'success'])(
    'retries real PostgreSQL serialization failure once for %s',
    async (mode) => {
      stage();
      ...
      try {
        ...
        sql(
          `DROP TRIGGER g2p_transient ON "ScoutReconstructionLedger"; DROP FUNCTION g2p_transient(); DROP SEQUENCE g2p_attempt`,
        );
      }
    },
  );
  ...
  it.each([false, true])('success holds claim while a failing %s old writer waits', async (old) => {
    ...
    try {
      ...
      expect(records()[0]).toMatchObject({
        status: old ? 'failed' : 'reconstructed',
        source_platform: 'truecoach',
      });
  ...
  it.each([false, true])('success holds claim while a skipped %s old writer waits', async (old) => {
    ...
    try {
      ...
      expect(records()[0]).toMatchObject({
        status: old ? 'skipped' : 'reconstructed',
        source_platform: 'truecoach',
      });
      expect(records()[0].target_id === null).toBe(old);
    } finally {
  ...
  it.each(['skip', 'throw'])(
    'T success upgrades serialized %s while target write is open',
    async (mode) => {
      stage();
  ...
  it('concurrent different-platform NULL claims serialize and the loser rolls back target changes', async () => {
    ...
    try {
      ...
      expect((await other.done).failure).toEqual({
        status: 409,
        message: 'reconstruction provenance conflict',
      });
...
describe('stage 4: Q0 real scoped reads, cursor boundaries and unchanged emission', () => {
  ...
  it.each(['clients', 'workouts'])(
    'enumerates exact narrow-schema %s page union and retains NULL legacy history',
    async (family) => {
      ...
      expect(
        (await run({ action, family, cursor: v2(family === 'clients' ? 'workouts' : 'clients') }))
          .failure,
  ...
  it.each(['clients', 'workouts'])(
    'characterizes %s ties ONLY in a temporary future-schema fixture',
    async (family) => {
  ...
  it.each(['clients', 'workouts'])(
    'bounds %s cursor tokens at 8,192 characters and rejects every malformed shape without reflection',
    async (family) => {
      stage('a', family);
      await run({ family });
      const action = actionOf(family);
      ...
      const malformed = [
        ...
        'v2.' +
          encoded(
            '{"v":2,"c":"coach","i":"intent","f":"' +
              family +
              '","o":"source_id:asc,source_platform:asc","s":"a\u0000","p":"truecoach"}',
          ),
        'v2.' +
          encoded(
            JSON.stringify({
        ...
        family === 'clients'
          ? encoded('a\u0000')
          : encoded(
              JSON.stringify({ c: 'coach', i: 'intent', f: 'clients', o: 'source_id:asc', s: 'a' }),
            ),
...
describe('stage 5: rollout recovery boundaries after T has claimed provenance', () => {
  ...
  it('refuses destructive E rollback while any claim exists, keeps history, and only a drained ledger can roll back', async () => {
    ...
    sql(`UPDATE "ScoutReconstructionLedger" SET source_platform=NULL`);
    const narrowRows = allLedger().map((r: any) => {
      const { source_platform: _source_platform, ...rest } = r;
      return rest;
    });
    sql(down);
    ...
    expect(
      sql(
        `SELECT count(*) FROM "_prisma_migrations" WHERE rolled_back_at IS NOT NULL OR finished_at IS NULL`,
      ),
    ).toBe('0');
  ...
  it('E/down lock budgets fail atomically against a live writer transaction instead of queueing behind it', async () => {
    ...
    const holder = holdTransaction(
      `UPDATE public."ScoutReconstructionLedger" SET reason='held' WHERE source_id='lock'`,
    );
    await holder.held;
    try {
      const t0 = Date.now();
  ...
  it('characterizes down against an actual T claim transaction paused inside its interactive transaction', async () => {
    ...
    try {
      ...
      if (branch === 'down-refused') {
        ...
        expect(hasColumn()).toBe('0');
        expect(
          sql(`SELECT count(*) FROM "ScoutReconstructionLedger" WHERE source_id='claimrace'`),
        ).toBe('0');
      }
      paused.release();
    ...
    expect(converged.map((r: any) => [r.source_id, r.status, r.source_platform])).toEqual([
      ['claimrace', 'reconstructed', 'truecoach'],
    ]);
  ...
  it("refuses down with a lock timeout while an actual T claim transaction outlives E's lock budget, and the released claim then commits intact", async () => {
    ...
    try {
      ...
      expect(appliedMigrations()).toBe('165');
      expect(
        sql(`SELECT count(*) FROM "ScoutReconstructionLedger" WHERE source_id='claimrace2'`),
      ).toBe('0');
      expect(targets()).toEqual({ persons: [], entities: [] });
      paused.release();
      outcome = await paused.done;
    ...
    const rows = records();
    expect(rows.map((r: any) => [r.source_id, r.status, r.source_platform, r.reason])).toEqual([
      ['claimrace2', 'reconstructed', 'truecoach', null],
    ]);

...
All notable changes to the `growth-project-backend` are documented here. Entries are
grouped by phase and dated at merge.
...
**Privacy:** every endpoint scopes to `req.user.id` only. No caller-supplied
coachId override is possible. Cross-coach isolation verified in
`test/coach-command-center.spec.ts`.

**Tests added:** `test/coach-command-center.spec.ts` — 14 assertions covering
role-guard rejection, cross-coach isolation (controller + service layers),
happy-path overview, empty roster, inbox sorting, action-queue filtering, and
pagination cursors.

---

*Earlier phases did not have CHANGELOG entries. Future phases will add entries here.*

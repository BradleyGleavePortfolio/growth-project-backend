...
Glue-catalog-compatible Parquet partitioning, verifies the PUT, and only then
deletes the archived rows. No row is ever deleted without an archive write. A
`--dry-run` mode prints the SQL and S3 paths without acting.

## GDPR Art. 17 erasure (D-H6-4)

Right-to-be-forgotten is NOT handled by deleting audit rows. Instead, the
`redactRowForErasure()` helper in `src/audit-log/erasure-token.ts` overwrites
the PII columns (`actor_id`, `ip_address`, `before_state`, `after_state`) in
place with the erasure token `[REDACTED:GDPR-ART-17]`, while leaving the audit
fact intact: the row's existence, action, resource_type, and timestamps
survive so the compliance trail (that something happened, when) is preserved.

Because the table REVOKEs UPDATE from the runtime role, the in-place erasure
patch is applied by the privileged erasure path, not the normal runtime role.

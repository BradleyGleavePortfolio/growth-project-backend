...
| [`masterminds-operating-model.md`](./masterminds-operating-model.md) | Draft business / product / operations spec for the paid IRL mastermind program (L2 / L3) layered on top of the per-seat coach SaaS: tier model, application + qualification funnel, IRL event lifecycle, curriculum, accountability pods, concierge setup, branded-instance roadmap, commercial model (deposits / refunds / rev-rec), product surfaces required, compliance + risk, operator runbooks, phased rollout. Spec only — no schema, code, or flag in scope. |

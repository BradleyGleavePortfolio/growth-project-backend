...
const touchedBilling = touched.some(
  (f) => f.startsWith('src/billing/') || f.startsWith('src/stripe'),
);
...
if (touchedMigrations)
  warn(
    '🛢 This PR touches **prisma/migrations/**. Confirm `down.sql` exists or `-- IRREVERSIBLE: <reason>` is present (R106).',
  );
if (deleted.some((f) => f.startsWith('prisma/migrations/')))
  warn(
    '🗑 This PR **deletes** files under **prisma/migrations/**. Deleting a shipped migration can desync environments — confirm this is intentional and coordinated (R106).',
  );
if (deleted.some((f) => /\bsrc\/auth\b/.test(f) || /\bsrc\/.*oauth\b/.test(f)))
  warn(
    '🗑 This PR **deletes** files under **src/auth**. Removing auth code can silently drop a security control — extra scrutiny.',
  );
if (deleted.some((f) => f.startsWith('src/billing/') || f.startsWith('src/stripe')))
  warn(
    '🗑 This PR **deletes** files under **src/billing**. Confirm no live payment path is being removed.',
  );
if (touchedPrismaSchema && !touchedMigrations)
  warn(
    '📐 You changed **prisma/schema.prisma** without a matching migration. Did you run `prisma migrate dev`?',
  );
if (touchedEnvValidation)
  warn('🔐 You changed **env-validation.ts**. Boot-time invariant change — extra scrutiny.');
if (touchedAuth)
  warn(
    '🔑 This PR touches **auth/oauth** code. Pair with a unit test that exercises an expired-token path.',
  );
if (touchedRls)
  warn('🛡 This PR touches **RLS / policy** code. Run `npm run test:rls-mwb2-concurrency` locally.');
if (touchedBilling)
  warn('💳 This PR touches **billing/stripe**. Idempotency keys and webhook signatures present?');
if (touchedWebhooks)
  warn(
    '📬 This PR touches **webhook** code. Replay-protected? Signature-verified? Returns 200 even on duplicate?',
  );
if (touchedRules)
  warn(
    '📜 This PR edits the **rule doctrine**. Operator review required. Mention rationale in PR body.',
  );

// ---------- 2) PR hygiene ----------
const CONVENTIONAL_RE =
  /^(feat|fix|perf|deps|revert|docs|refactor|test|build|ci|chore)(\([^)]+\))?!?:\s/;
...
const conventionalSubject = CONVENTIONAL_RE.test(prTitle)
  ? prTitle
  : CONVENTIONAL_RE.test(latestCommitSubject)
    ? latestCommitSubject
    : '';
...
if (!prTitle && !latestCommitSubject) {
  ...
  warn(
    'Could not read the PR title or commit subjects to validate Conventional Commits format. Verify the title parses for release-please.',
  );
...
if (prBody.length < 80) {
  warn(
    'PR description is short. Use the PR template — describe what changed, why, and the test plan.',
  );
}
...
if (prBody.includes('BREAKING CHANGE:')) {
  warn(
    'This PR contains a **BREAKING CHANGE:** footer. Confirm the major version bump is intentional and the migration/rollback path is documented.',
  );
}
...
schedule(async () => {
  ...
  for (const f of touched.filter((f) => /\.(ts|tsx|js)$/.test(f))) {
    try {
      ...
      warn(
        `Could not diff ${f} for TODO density: ${err && err.message ? err.message : String(err)}`,
      );
    }
  }
  if (n >= 5)
    warn(
      `This PR introduces **${n}** new TODO/FIXME/XXX/HACK markers. Track them in BACKLOG.md or an issue, not as drive-by markers.`,
    );
});
...
const summary = [
  `**Touched areas:** ${
    [
      ...
      .filter(Boolean)
      .join(', ') || '_none of the sensitive paths_'
  }`,
